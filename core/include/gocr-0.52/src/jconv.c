...
void help(void) {
  printf(
      "jconv version Aug2000 JS (pnm-raw,pcx8,tga24)\n"
...
int main(int argn, char *argv[]) {
  ...
  // ----- read picture
  if (strstr(inam, ".pbm") || strstr(inam, ".pgm") || strstr(inam, ".ppm") ||
      strstr(inam, ".pnm") || strstr(inam, ".pam"))
    readpgm(inam, &bild, 1);
  ...
  if ((vvv & 2) == 2 && bild.bpp == 1) {  // -shrink
    ...
    for (y = 0; y < dy; y++) {  // shrink upper border
      for (x = 0; x < dx; x++)
        if (bild.p[x + ox + (y + oy) * bild.x] < 127) break;
      if (x < dx) {
        if (y > 0) y--;
        oy += y;
        dy -= y;
    ...
    for (y = 0; y < dy; y++) {  // shrink lower border
      for (x = 0; x < dx; x++)
        if (bild.p[ox + x + (oy + dy - y - 1) * bild.x] < 127) break;
      if (x < dx) {
        if (y > 0) y--;
        dy -= y;
        break;
    ...
    for (x = 0; x < dx; x++) {  // shrink right border
      for (y = 0; y < dy; y++)
        if (bild.p[ox + dx - x - 1 + (oy + y) * bild.x] < 127) break;
      if (y < dy) {
        if (x > 0) x--;
        dx -= x;
        break;
  ...
  if (strstr(onam, ".pbm"))
    ...
    printf("commented out line writeppm should have been used...\n");
    exit(1);
    // writeppm(onam, &bild);
