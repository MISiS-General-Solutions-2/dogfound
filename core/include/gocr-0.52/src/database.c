...
int load_db(job_t *job) {  // called by gocr.c main()
  ...
  for (ii = 0; !feof(f1); ii++) {
    ...
    if (s1[i] == '"') { /* parse a string */
      j = strrchr(s1 + i + 1, '"') -
          (s1 + i + 1); /* we only look for first and last "" */
      if (j >= 1) {
        s3 = (char *)malloc(j + 1);
        ...
        fprintf(stderr, "load_db: string parse error L%d\n", line);
      }
    } else {
  ...
  fclose(f1);
  if (job->cfg.verbose) fprintf(stderr, " %d chars loaded\n", ii);
  return 0;
}
...
int store_db(struct box *box1, job_t *job) {
  ...
  if (job->cfg.verbose)
    fprintf(stderr,
            "store_db: add file %s to database (nac=%d c=%04x)"
            "\n#",
  ...
  if (copybox(box1->p, box1->x0, box1->y0, dx, dy, &b, dx * dy, job)) {
    fclose(f1);
    return -1;
  }
...
void out_env(struct box *px, job_t *job) {
  ...
  { /* overwrite rest of arguments */
    ...
    fprintf(stderr, "\n# show box     x= %4d %4d d= %3d %3d r= %d %d", px->x0,
            px->y0, px->x1 - px->x0 + 1, px->y1 - px->y0 + 1, px->x - px->x0,
            px->y - px->y0);
  ...
  if (dx > 0)
    for (y = yy0; y < yy0 + dy; y += ty) { /* reduce the output to max 78x40 */
      ...
      for (x = x0; x < x0 + dx; x += tx) { /* by merging sub-pixels */
        ...
        for (y2 = y; y2 < y + ty && y2 < y0 + dy; y2++) /* sub-pixels */
          for (x2 = x; x2 < x + tx && x2 < x0 + dx; x2++) {
            if ((getpixel(b, x2, y2, job) < cs)) c1 = '#';
          }
        // show pixels outside the box thinner/weaker
        if (x + tx - 1 < px->x0 || x > px->x1 || y + ty - 1 < px->y0 ||
            y > px->y1)
          c1 = ((c1 == '#') ? 'O' : ',');
        fprintf(stderr, "%c", c1);
      }
      ...
      /* mark lines with < */
      if (px)
        if (y == px->m1 || y == px->m2 || y == px->m3 || y == px->m4) c1 = '<';
...
*/
// #define DEBUG 2
int distance2(pix *p1, struct box *box1, pix *p2, struct box *box2, int cs,
              int vvv, job_t *job) {
  int rc = 0, x, y, v1, v2, i1, i2, rgood = 0, rbad = 0, x1, y1, x2, y2, dx, dy,
      dx1, dy1, dx2, dy2, tx, ty;
#if DEBUG == 2
  if (vvv) fprintf(stderr, " DEBUG: distance2\n");
#endif
  x1 = box1->x0;
  y1 = box1->y0;
  x2 = box2->x0;
  ...
  dy = ((dy1 > dy2) ? dy1 : dy2);
  dy = dy1;
  if (abs(dx1 - dx2) > 1 + dx / 16 || abs(dy1 - dy2) > 1 + dy / 16)
    rbad++;  // how to weight?
  // compare relations to baseline and upper line
  if (box1->m4 > 0 && box2->m4 > 0) {  // used ???
    if (2 * box1->y1 > box1->m3 + box1->m4 &&
        2 * box2->y1 < box2->m3 + box2->m4)
      rbad += 128;
    if (2 * box1->y0 > box1->m1 + box1->m2 &&
        2 * box2->y0 < box2->m1 + box2->m2)
      rbad += 128;
  }
  tx = dx / 16;
  if (dx < 17) tx = 1;  // raster
  ty = dy / 32;
  if (dy < 33) ty = 1;
  // compare pixels
  for (y = 0; y < dy; y += ty)
    for (x = 0; x < dx; x += tx) {  // try global shift too ???
      v1 = ((getpixel(p1, x1 + x * dx1 / dx, y1 + y * dy1 / dy, job) < cs) ? 1
                                                                           : 0);
      i1 = 8;  // better gray?
      v2 = ((getpixel(p2, x2 + x * dx2 / dx, y2 + y * dy2 / dy, job) < cs) ? 1
                                                                           : 0);
      i2 = 8;  // better gray?
      if (v1 == v2) {
        rgood += 16;
      ...
      for (i1 = -1; i1 < 2; i1++)
        for (i2 = -1; i2 < 2; i2++)
          if (i1 != 0 || i2 != 0) {
            if (((getpixel(p1, x1 + x * dx1 / dx + i1 * (1 + dx1 / 32),
                           y1 + y * dy1 / dy + i2 * (1 + dy1 / 32), job) < cs)
                     ? 1
                     : 0) !=
                ((getpixel(p2, x2 + x * dx2 / dx + i1 * (1 + dx2 / 32),
                           y2 + y * dy2 / dy + i2 * (1 + dy2 / 32), job) < cs)
                     ? 1
                     : 0))
              v1++;
          }
...
wchar_t ocr_db(struct box *box1, job_t *job) {
      ...
      if (dd <= dist) { /* new best fit */
        ...
        if (dist < 100 && 100 - dist >= job->cfg.certainty) {
          ...
          for (i = 0; i < box3->num_ac; i++) {
            ...
            if (box3->tas[i])
              setas(box1, box3->tas[i], wa, job);
            else
              setac(box1, box3->tac[i], wa, job);
          }
          ...
          if (job->cfg.verbose)
            fprintf(stderr, " dist=%4d c= %c 0x%02x %s  wc= %3d", dist,
                    ...
                    (int)box3->c, ((box3->tas[0]) ? box3->tas[0] : ""),
                    box3->wac[0]);
        }
  ...
  if ((job->cfg.mode & 128) != 0 && c == UNKNOWN) { /* prompt the user */
    ...
    fprintf(
        ...
        "Press RET at the end (ALT+RET to store into RAM only) : "); /* ToDo:
                                                                        empty +
                                                                        alt-return
                                                                        (0x1b
                                                                        0x0a)
    ...
    for (i = 0; i < dd; i++)
      if (buf[i] < 32) break; /* need unsigned char here */
    ...
    if (dd > 1 && dd < 7) { /* try to decode single wide char (utf8) */
      ...
      for (i = 1; i < dd; i++)
        if ((buf[i] & 0xc0) != 0x80) break; /* 10xxxxxx */
      if (i == dd && (buf[0] & (u0 | u1)) == u1) {
        utf8_ok = 1;
        c = buf[0] & (u0 - 1); /* 11..0x.. */
        for (i = 1; i < dd; i++) {
          c <<= 6;
