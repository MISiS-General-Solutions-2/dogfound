...
#if defined(O_BINARY) &&                                           \
    (defined(__WIN32) || defined(__WIN32__) || defined(__WIN64) || \
     defined(__WIN64__) || defined(__MSDOS__))
#include <fcntl.h>
#define SET_BINARY(_f)                      \
  do {                                      \
    if (!isatty(_f)) setmode(_f, O_BINARY); \
  } while (0)
#else
...
char *xlist[] = {
    ...
    ".pbm.gz", "gzip -cd", ".pgm.gz", "gzip -cd", ".ppm.gz", "gzip -cd",
    ".pnm.bz2", "bzip2 -cd", ".pbm.bz2", "bzip2 -cd", ".pgm.bz2", "bzip2 -cd",
    ".ppm.bz2", "bzip2 -cd", ".jpg",
    "djpeg -gray -pnm", /* JPG/JPEG, jpeg package */
    ".jpeg", "djpeg -gray -pnm", ".gif",
    "giftopnm -image=all", /* GIF, netpbm package */
    ".bmp", "bmptoppm", ".tiff", "tifftopnm", ".png",
    "pngtopnm", /* Portable Network Graphics (PNG) format */
...
int readpgm(char *name, pix *p, int vvv) {
  ...
  if (vvv) {
    fprintf(stderr, "# readpam: format=0x%04x=%c%c h*w(d*b)=%d*%d(%d*%d)\n",
            ...
            ((magic2 > 31 && magic2 < 127) ? magic2 : '.'), inpam.height,
            inpam.width, inpam.depth, inpam.bytes_per_sample);
  }
  ...
  for (i = 0; i < inpam.height; i++) {
    ...
    for (j = 0; j < inpam.width; j++) {
      if (inpam.depth >= 3) /* tuplerow is unsigned long (see pam.h sample) */
        /* we expect 8bit or 16bit integers,
           no overflow up to 32-10-2=20 bits */
        sample = ((PPM_RED_WEIGHT * tuplerow[j][0] + 511) >> 10) +
...
static int fread_num(char *buf, int bps, FILE *f1) {
  ...
  for (mode = 0;
       !feof(f1);) {  // mod=0 means skip leading spaces, 1 scan digits
    c1 = read_char(f1);
    if (isspace(c1)) {
      if (mode == 0)
        continue;
...
int readpgm(char *name, pix *p, int vvv) {
  ...
  if (vvv)
    fprintf(stderr, "# PNM P%c h*w=%d*%d c=%d head=%ld", c2, ny, nx, nc,
            ftell(f1));
  if (c2 == '4' && (nx & 7) != 0) {
    /* nx=(nx+7)&~7;*/ if (vvv)
      fprintf(stderr, " PBM2PGM nx %d", (nx + 7) & ~7);
  }
  if (nc >> 8) bps = 2;  // bytes per color and pixel
  if (nc >> 16) bps = 3;
  if (nc >> 24) bps = 4;
  fflush(stdout);
  if ((1. * (nx * ny)) != ((1. * nx) * ny)) F0("Error integer overflow");
  ...
  if (c2 == '5' || c2 == '2') /* slow PGM-RAW/ASC read pixelwise */
    for (i = 0; i < nx * ny; i++) {
      ...
      // P2 was buggy until 2018-09 read 3 values per pixel, fixed by F.O.
        ...
        fread_num(buf, bps, f1);  // reading one number per pixel
  ...
  if (c2 == '6' || c2 == '3') {  // PPM-RAW/ASC
    for (i = 0; i < nx * ny; i++) {
      if (c2 == '6') {
        if (3 * bps != (int)fread(buf, 1, 3 * bps, f1)) {
          fprintf(stderr, " ERROR reading at head+3*%d*%d\n", bps, i);
          break;
        }
      } else
        for (j = 0; j < 3; j++) fread_num(buf + j * bps, bps, f1);
      pic[i] =
  ...
  if (c2 == '1')
    for (mod = j = i = 0, nc = 255;
         i < nx * ny && !feof(f1);) {  // PBM-ASCII 0001100
      c1 = read_char(f1);
      if (isdigit(c1)) {
        pic[i] = ((c1 == '0') ? 255 : 0);
        i++;
...
char read_char_from_pchar(char *data, int datalen,
                          int *pos) {  // filter #-comments
  char c;
  int m;
  for (m = 0;;) {
    if (*pos == datalen) {
...
int readPgmFromMemory(int datalen, char *data, pix *p, int vvv) {
  static char c1, c2;         /* magic bytes, file type */
  int nx, ny, nc, mod, i, j;  // buffer
  unsigned char *pic;
  char buf[512];
  ...
  for (mod = 0; ((c2 == '5' || c2 == '2') && (mod & 7) < 6) ||
                ...
                ((c2 == '4' || c2 == '1') &&
                 (mod & 7) < 4);) {  // mode: 0,2,4=[ |\t|\r|\n]
                                     //   1=nx 3=ny 5=nc 8-13=#rem
    c1 = read_char_from_pchar(data, datalen, &pos);
    if (vvv) fprintf(stdout, "%d\n", c1);
    if ((mod & 1) == 0)  // whitespaces
      if (!isspace(c1)) mod++;
    if ((mod & 1) == 1) {
  ...
  if (vvv) fprintf(stdout, "finished outputing image\n", pic[i]);
  fflush(stdout);
  // if (vvv) fprintf(stderr, "\nassigned &data[%d] go pic", pos);
  {
    int minc = 255, maxc = 0;
    for (i = 0; i < nx * ny; i++) {
      if (pic[i] > maxc) maxc = pic[i];
...
int writepgm(char *nam, pix *p) {  // P5 raw-pgm
  ...
  if (p->y != (int)fwrite(p->p, p->x, p->y, f1))
    F0("write");  // write all lines
  fclose(f1);
  return 0;
}
...
void addrgb(unsigned char rgb[3], int sr, int sg, int sb) {
  ...
  for (i = 0; i < 3; i++) /* avoid overflow */
    if (add[i] < 0)
      rgb[i] = ((rgb[i] + add[i] < 0) ? 0 : rgb[i] + add[i]);
    else
      rgb[i] = ((rgb[i] + add[i] > 255) ? 255 : rgb[i] + add[i]);
}
/*
...
int writepbm(char *nam, pix *p) {  // P4 raw-pbm
  ...
  for (y = 0; y < p->y; y++)
    for (x = 0; x < p->x; x++) {  // set bit
      a = (x + y * dx) >> 3;
      b = 7 - (x & 7);  // adress an bitisnumber
      i = x + y * p->x;
      if (p->bpp == 3)
        i = (p->p[3 * i + 0] + p->p[3 * i + 1] + p->p[3 * i + 2]) / 3;
      else
        i = p->p[i];
      i = ((i > 127) ? 0 : 1);
      p->p[a] = (p->p[a] & (~1 << b)) | (i << b);
  ...
  if (!f1) F0("open");  // open-error
  fprintf(f1, "P4\n%d %d\n", p->x, p->y);
  if (p->y != (int)fwrite(p->p, dx >> 3, p->y, f1))
    F0("write");  // write all lines
  fclose(f1);
  return 0;
}
