...
int get_line(int x0, int y0, int x1, int y1, pix *p, int cs, int ret,
             job_t *job) {
  int dx, dy, incrE, incrNE, d, x, y, r0, r1, ty, tx, *px, *py, *pdx, *pdy,
      *ptx, *pty, *px1;
  dx = abs(x1 - x0);
  tx = ((x1 > x0) ? 1 : -1);  // tx=x-spiegelung (new)
  dy = abs(y1 - y0);
  ty = ((y1 > y0) ? 1 : -1);  // ty=y-spiegelung (new)
  ...
  x = x0;
  y = y0;
  r0 = r1 = 0; /* dd=tolerance (store max drift) */
  while ((*px) <= (*px1)) {
    if (((getpixel(p, x, y, job) < cs) ? 1 : 0) ^ (ret & 1))
      r0++;
    else
      r1++;
    (*px)++;
...
int get_line2(int x0, int y0, int x1, int y1, pix *p, int cs, int ret,
              job_t *job) {
  int dx, dy, incrE, incrNE, d, x, y, r0, r1, ty, tx, q, ddy, rx, ry, *px, *py,
      *pdx, *pdy, *ptx, *pty, *px1;
  dx = abs(x1 - x0);
  tx = ((x1 > x0) ? 1 : -1);  // tx=x-spiegelung (new)
  dy = abs(y1 - y0);
...
char get_bw(int x0, int x1, int y0, int y1, pix *p, int cs, int mask,
  ...
  for (y = y0; y <= y1; y++)
    for (x = x0; x <= x1; x++) {
      rc |= ((getpixel(p, x, y, job) < cs) ? 1 : 2);  // break if rc==3
 ...
 */
int num_cross(int x0, int x1, int y0, int y1, pix *p, int cs, job_t *job) {
  ...
  for (i = 0, x = x0, y = y0; i <= d; i++) {
    ...
    k = ((getpixel(p, x, y, job) < cs) ? 1 : 0);  // 0=white 1=black
    ...
    if (col == 1 && k == 0) {
      if ((w2_cross <= 1 && w1_white <= 1 && w1_cross > 7) ||
          (w1_cross <= 1 && w1_white <= 1 && w2_cross > 7))
        if (rc > 1) rc--;  // 1810 remove noise
...
int num_cross_fine(int x0, int x1, int y0, int y1, pix *p, int cs, job_t *job) {
  int rc = 0, col = 0, k, x, y, i, d;  // rc=crossings  col=0=white
  int dx = x1 - x0, dy = y1 - y0;
...
int setas(struct box *b, char *as, int weight, job_t *job) {
  int i, j;
...
int setac(struct box *b, wchar_t ac, int weight, job_t *job) {
  int i, j;
  if ((!b) || b->num_ac > NumAlt || b->num_ac < 0) {
    fprintf(stderr, "\nDBG: This is a bad call to setac()!");
    if (b && (job->cfg.verbose & 6)) out_x(b, job);
    b->num_ac = 0;
  }
  if (ac == 0 || ac == UNKNOWN) {
...
int follow_path(int x0, int x1, int y0, int y1, pix *p, int cs, path_t *path,
  ...
  d = MAX(abs(dx), abs(dy));
  prev = getpixel(p, x0, y0, job) < cs;  // 0=white 1=black
  path->start = prev;
  for (i = 1, x = x0, y = y0; i <= d; i++) {
    ...
    color = getpixel(p, x, y, job) < cs;  // 0=white 1=black
    if (color != prev) {
      if (rc >= path->max) {
        int n = path->max * 2 + 10;
...
void turmite(pix *p, int *x, int *y, int x0, int x1, int y0, int y1, int cs,
  ...
  while (*x >= x0 && *y >= y0 && *x <= x1 && *y <= y1) {
    r = ((getpixel(p, *x, *y, job) < cs) ? rb : rw);  // select rule
    switch (r) {
      case UP:
        (*y)--;
        break;
...
int joined(pix *p, int x0, int y0, int x1, int y1, int cs, job_t *job) {
  ...
  t = ((getpixel(p, x, y, job) < cs) ? 1 : 0);
  for (;;) {
    if (t == ((getpixel(p, x + dy, y - dx, job) < cs) ? 1 : 0)  // right free?
        && x + dy >= xa && x + dy <= xb && y - dx >= ya &&
        y - dx <= yb)  // wall
    {
      r = dy;
      dy = -dx;
    ...
    // fprintf(stderr," path xy %d-%d %d-%d %d %d  %d
...
int loop(pix *p, int x, int y, int l, int cs, int col, DIRECTION r,
  ...
  if (x >= 0 && y >= 0 && x < p->x && y < p->y) {
    switch (r) {
      case UP:
        for (; i < l && y >= 0; i++, y--)
          if ((getpixel(p, x, y, job) < cs) ^ col) break;
        break;
      case DO:
        for (; i < l && y < p->y; i++, y++)
          if ((getpixel(p, x, y, job) < cs) ^ col) break;
 ...
 *
 * looking for better algo: go horizontally and look for upper/lower
 * non_marked_pixel/nopixel use lowest three bits for mark
 ...
 */
int frame_nn(pix *p, int x, int y, int *x0, int *x1, int *y0,
             int *y1,  // enlarge frame
             int cs, int mark, int diag, job_t *job) {
#if 1 /* flood-fill to detect black objects, simple and faster? */
  int rc = 0, dx, col, maxstack = 0;
  static int overflow = 0;
  int bmax = 1024, blen = 0,
      *buf; /* buffer as replacement for recursion stack */
  ...
  g_debug(fprintf(stderr, "\nframe_nn   x=%4d y=%4d", x, y);) for (; blen;) {
    ...
    for (; x > 0 && (col == ((getpixel(p, x - 1, y, job) < cs) ? 0 : 1)); x--)
      ;
    ...
    for (dx = -1; dx < 2; dx += 2) /* look at upper and lower line, left */
      if (diag && x < p->x && x - 1 > 0 && y + dx >= 0 && y + dx < p->y &&
          col != ((getpixel(p, x, y + dx, job) < cs) ? 0 : 1) &&
          col == ((getpixel(p, x - 1, y + dx, job) < cs) ? 0 : 1) &&
    ...
    for (; x < p->x && (col == ((getpixel(p, x, y, job) < cs) ? 0 : 1)); x++) {
      p->p[x + y * p->x] |= (mark & 7);
      rc++; /* mark pixel */
      /* enlarge frame */
      if (x > *x1) *x1 = x;
      for (dx = -1; dx < 2; dx += 2) /* look at upper and lower line */
        if (col == ((getpixel(p, x, y + dx, job) < cs) ? 0 : 1) &&
            (col != ((getpixel(p, x - 1, y, job) < cs) ? 0 : 1) ||
             col != ((getpixel(p, x - 1, y + dx, job) < cs) ? 0 : 1)) &&
            !((marked(p, x, y + dx) & mark) == mark) && y + dx < p->y &&
            y + dx >= 0) {
          if (blen + 1 >= bmax) {
            overflow |= 1;
            continue;
  ...
  g_debug(fprintf(stderr, " maxstack= %4d pixels= %6d", maxstack,
                  rc);) if (overflow == 1) {
    overflow |= 2;
  ...
  static const int d0[8][2] = {
      {0, -1} /* up    */, {-1, -1}, {-1, 0} /* left  */, {-1, 1},
      {0, 1} /* down  */,  {1, 1},   {1, 0} /* right */,  {1, -1}};
  ...
  g_debug(fprintf(stderr, " start frame:");)
    ...
    g_debug(fprintf(stderr, " ln=%d diag=%d cs=%d x=%d y=%d - go to border\n",
        ...
        od = d =
            (8 + 4 * ln - diag) & 7;  // start robot looks up, right is a wall
    ...
    for (rot = 0; abs(rot) <= 64;) { /* for sure max. 8 spirals */
      ...
      g_debug(fprintf(stderr, " xy %3d %3d d=%d i=%d p=%3d rc=%d\n", x, y, d, i,
        ...
        for (j = 0, dx = d0[d][1]; x + j >= 0 && x + j < p->x &&
                                   i == ((getpixel(p, x + j, y) < cs) ? 0 : 1);
             j += dx) {
          if (!((marked(p, x + j, y) & mark) == mark)) rc++;
...
int mark_nn(pix *p, int x, int y, int cs, int r, job_t *job) {
  /* out of bounds or already marked? */
  if (outbounds(p, x, y) || (marked(p, x, y) & r) == r) return 0;
  {
    int x0, x1, y0, y1;
    x0 = x1 = x;
    y0 = y1 = y;  // not used
...
int frame_vector(struct box *box1, int x, int y, int cs, int mark, int diag,
                 int ds, job_t *job) {
  int i1, i2, i2o, new_x = 1, /* flag for storing the vector x,y */
   ...
   */
  static const int d0[8][2] = {{0, -1}, /* up    */ {-1, -1}, /* up-le */
                               {-1, 0}, /* left  */ {-1, 1},  /* do-le */
                               {0, 1},  /* down  */ {1, 1},   /* do-ri */
                               {1, 0},  /* right */ {1, -1}}; /* up-ri */
  ...
  d = ds;
  i1 = ((getpixel(p, x, y, job) < cs) ? 0 : 1);
  i2 = ((getpixel(p, x + d0[d][0], y + d0[d][1], job) < cs) ? 0 : 1);

  g_debug(fprintf(stderr,
                  "\nLEV2 frame_vector @ %3d %3d  d%d %2d %2d"
                  "  %d-%d pix=%3d mark=%d cs=%d",
                  x, y, ds, d0[ds][0], d0[ds][1], i1, i2, getpixel(p, x, y),
                  mark, cs);)
  ...
  */
  ...
  /* index to next (x,y) */
  i2o = i2 =
      ((box1->num_frames == 0) ? 0 : box1->num_frame_vectors[box1->num_frames]);
#if 0  // obsolete v0.43
  ...
  /* robot stores start-position */
  ox = x;
  oy = y; /* look forward to white pixel */

  for (;;) { /* stop if same marked pixel touched */

    g_debug(fprintf(stderr, "\nLEV3: xy %3d %3d d= %d rot= %2d  %3d", x, y, d,
                    rot, i2);)
    ...
    if (outbounds(p, nx, ny) ||
        i1 != ((getpixel(p, nx, ny, job) < cs) ? 0 : 1)) {
  ...
  g_debug(fprintf(stderr, "\nLEV2 o= %3d %3d  xy %3d %3d  r=%d v=%d", ox, oy, x,
                  y, rot, vol);)
...
int num_hole(int x0, int x1, int y0, int y1, pix *p, int cs, holes_t *holes,
  ...
  if (copybox(p, x0, y0, dx, dy, &b, dx * dy, job)) {
    free(b.p);
    return -1;
  }
  ...
  for (x = 0; x < b.x; x++) {
    if (getpixel(&b, x, 0, job) >= cs) mark_nn(&b, x, 0, cs, AT, job);
    if (getpixel(&b, x, b.y - 1, job) >= cs)
      mark_nn(&b, x, b.y - 1, cs, AT, job);
  }
  for (y = 0; y < b.y; y++) {
    if (getpixel(&b, 0, y, job) >= cs) mark_nn(&b, 0, y, cs, AT, job);
    if (getpixel(&b, b.x - 1, y, job) >= cs)
      mark_nn(&b, b.x - 1, y, cs, AT, job);
  }
  ...
  g_debug(out_b(NULL, &b, 0, 0, b.x, b.y, cs);)
      ...
      for (x = 0; x < b.x;
           x++) for (y = 0; y < b.y;
                     y++) if (!((marked(&b, x, y) & AT) == AT))  // unmarked
      if (getpixel(&b, x, y, job) >= cs) {                       // hole found
#if 0
    ...
    { /* new version, for future store of hole characteristics */
      ...
      x0 = x1 = x;
      y0 = y1 = y;  // not used
      hole_size = frame_nn(&b, x, y, &x0, &x1, &y0, &y1, cs, AT,
                           job->tmp.n_run & 1, job);
...
int num_obj(int x0, int x1, int y0, int y1, pix *p, int cs, job_t *job) {
  ...
  if (!buf) {
    fprintf(stderr, "\nFATAL: malloc(%d) failed, skip num_obj",
            (x1 - x0 + 1) * (y1 - y0 + 1));
    return 0;
  }
  if (copybox(p, x0, y0, x1 - x0 + 1, y1 - y0 + 1, &b,
              (x1 - x0 + 1) * (y1 - y0 + 1), job)) {
    free(b.p);
    return -1;
  }
  ...
  for (x = 0; x < b.x; x++)
    for (y = 0; y < b.y; y++)
      if (getpixel(&b, x, y, job) < cs)
        if (!((marked(&b, x, y) & AT) == AT)) {
          rc++;
          mark_nn(&b, x, y, cs, AT, job);
        }
  free(b.p);
  return rc;
}
...
int distance(pix *p1, struct box *box1, pix *p2, struct box *box2, int cs,
             job_t *job) {
  int rc = 0, x, y, v1, v2, i1, i2, rgood = 0, rbad = 0, x1, y1, x2, y2, dx, dy,
      dx1, dy1, dx2, dy2;
  x1 = box1->x0;
  y1 = box1->y0;
  x2 = box2->x0;
  ...
  if (2 * box1->y1 > box1->m3 + box1->m4 && 2 * box2->y1 < box2->m3 + box2->m4)
    rbad += 128;
  if (2 * box1->y0 > box1->m1 + box1->m2 && 2 * box2->y0 < box2->m1 + box2->m2)
    rbad += 128;
  // compare pixels
  for (y = 0; y < dy; y++)
    for (x = 0; x < dx; x++) {  // try global shift too ???
      v1 = ((getpixel(p1, x1 + x, y1 + y, job) < cs) ? 1 : 0);
      i1 = 8;  // better gray?
      v2 = ((getpixel(p2, x2 + x, y2 + y, job) < cs) ? 1 : 0);
      i2 = 8;  // better gray?
      if (v1 == v2) {
        rgood += 8;
        continue;
      ...
      for (i1 = -1; i1 < 2; i1++)
        for (i2 = -1; i2 < 2; i2++)
          if (i1 != 0 || i2 != 0) {
            if (((getpixel(p1, x1 + x + i1 * (1 + dx / 32),
                           y1 + y + i2 * (1 + dy / 32), job) < cs)
                     ? 1
                     : 0) != ((getpixel(p2, x2 + x + i1 * (1 + dx / 32),
                                        y2 + y + i2 * (1 + dy / 32), job) < cs)
                                  ? 1
                                  : 0))
              v1++;
          }
  ...
  if (rgood + rbad)
    rc = (100 * rbad + (rgood + rbad - 1)) / (rgood + rbad);
  else
    rc = 99;
...
wchar_t whatletter(struct box *box1, int cs, int nrun, job_t *job) {
  ...
  while (get_bw(x0, x1, y0, y0, p, cs, 1, job) != 1 && y0 + 1 < y1) y0++;
  while (get_bw(x0, x1, y1, y1, p, cs, 1, job) != 1 && y0 + 1 < y1) y1--;
  dx = x1 - x0 + 1;
  dy = y1 - y0 + 1;  // size
  ...
  if (um == SPACE && dy > 5 && box1->num_boxes > 1)
    testumlaut(box1, cs, 2, &um, job); /* set box1->modifier + new y0 */
  ...
  while (get_bw(x0, x1, y0, y0, p, cs, 1, job) == 0 && y0 + 1 < y1) y0++;
  while (get_bw(x0, x1, y1, y1, p, cs, 1, job) == 0 && y0 + 1 < y1) y1--;
  while (get_bw(x0, x0, y0, y1, p, cs, 1, job) == 0 && x0 + 1 < x1) x0++;
  while (get_bw(x1, x1, y0, y1, p, cs, 1, job) == 0 && x0 + 1 < x1) x1--;
  dx = x1 - x0 + 1;
  dy = y1 - y0 + 1;  // size
  box1->x0 = x0;
  box1->y0 = y0;  // set reduced frame
  box1->x1 = x1;
  box1->y1 = y1;

  // set good startpoint (probably bad from division)?
  if (xa < x0 || xa > x1 || ya < y0 || ya > y1 ||
      getpixel(p, xa, ya, job) >= cs /* || 2*ya<y0+y1 */ || dots > 0) {
    // subfunction? also called after division of two glued chars?
    for (y = y1; y >= y0; y--)  // low to high (not i-dot)
      for (x = (x0 + x1) / 2, i = 0; x >= x0 && x <= x1;
           i++, x += ((2 * i & 2) - 1) * i) /* is that ok? */
        if (getpixel(p, x, y, job) < cs && (getpixel(p, x + 1, y, job) < cs ||
                                            getpixel(p, x, y + 1, job) < cs)) {
          xa = x;
          ya = y;
          y = -1;
          break;
  ...
  if (y0 > 0)  // mark upper overlap
    for (x = x0; x <= x1; x++) {
      if (getpixel(p, x, y0 - 1, job) < cs && getpixel(p, x, y0, job) < cs &&
          (marked(&b, x - x0, 0) & 1) != 1)
        mark_nn(&b, x - x0, 0, cs, 1, job);
    }
  if (x0 > 0)  // mark left overlap
    for (y = y0; y <= y1; y++) {
      if (getpixel(p, x0 - 1, y, job) < cs && getpixel(p, x0, y, job) < cs &&
          (marked(&b, 0, y - y0) & 1) != 1)
        mark_nn(&b, 0, y - y0, cs, 1, job);
    }
  if (x1 < p->x - 1)  // mark right overlap
    for (y = y0; y <= y1; y++) {
      if (getpixel(p, x1 + 1, y, job) < cs && getpixel(p, x1, y, job) < cs &&
          (marked(&b, x1 - x0, y - y0) & 1) != 1)
        mark_nn(&b, x1 - x0, y - y0, cs, 1, job);
    }
  mark_nn(&b, xa - x0, ya - y0, cs, 2, job);  // not glued chars
  for (x = 0; x < b.x; x++)
    for (y = 0; y < b.y; y++) {
      if ((marked(&b, x, y) & 3) == 1 && getpixel(&b, x, y, job) < cs)
        b.p[x + y * b.x] = 255 & ~7; /* reset pixel */
    }
#endif

  // if (bc == UNKNOWN)   // cause split to fail
  bc = ocr0(box1, &b, cs, job);
...
int scan_boxes(job_t *job, pix *p) {
  ...
  if (job->cfg.verbose) fprintf(stderr, "# scan_boxes");
  ...
  for (y = 0; y < p->y; y++)
    for (x = 0; x < p->x; x++)         // ds = direction to go 2=left 6=right
      for (ds = 2; ds < 7; ds += 4) {  // NO - dust of size 1 is not removed !!!
        ...
        if (getpixel(p, x, y, job) >= cs ||
            getpixel(p, nx, y, job) < cs)  // b/w transition?
          continue;
        if ((marked(p, x, y) & 1) && (marked(p, nx, y) & 1)) continue;
         ...
         *  ds   - start direction, 6=right of right border, 2=left of left
         * border ret  - 0=ok, -1=already marked, -2=max_num_frames_exceeded
         *               -7=no border in direction ds
         *  ToDo: count errors and print out for debugging
         */
        rc = frame_vector(box3, x, y, cs, 1, 1, ds, job);
        g_debug(fprintf(stderr, "\n# ... scan xy= %3d %3d rc= %2d", x, y,
                        rc);) if (rc < 0) {
          free_box(box3);
          continue;
        }
        if (box3->num_frames && !box3->num_frame_vectors[0])
          fprintf(stderr, "\nERROR scan_boxes: no vector in frame (%d,%d)", x,
                  y);
        ...
        box3->c =
            (((box3->y1 - box3->y0 + 1) * (box3->x1 - box3->x0 + 1) >= MaxBox)
                 ? PICTURE
                 : UNKNOWN);
 ...
 *   2nd round max. mono_width = min x0-pre.x0, x1-pre.x1 (+check against
 * min_mono_em) 3th round if something not fit, mono=0
 *
 */
void measure_pitch(job_t *job) {        /* word spacing */
  ...
  if (job->cfg.verbose) {
    fprintf(stderr, "# check for word pitch");
  }
  for (l1 = 0; l1 < job->res.lines.num; l1++) { /* 0 means all lines */
    if (job->cfg.verbose) {
      fprintf(stderr, "\n#  line %2d\n# ...", l1);
    }
    ...
    monospaced = 1;
    mono_em_min = 0;
    mono_em_max = 2047;  // reset, 2010-09-28
    char_width_min = 1023;
    char_width_max = 0;  // reset, 2010-09-28
    for_each_data(&(job->res.boxlist)) {
      ...
      if (pre1) {                   // 2010-09-28
        ...
        if (d1l > d1r) {
          d1 = d1r;
          d2 = d1l;
        }  // thinner char on the right
        else {
          d1 = d1l;
          d2 = d1r;
        }  // thicker char on the right
        ...
        if ((48 & job->cfg.verbose) == 48)
          if (monospaced && l1)  // debugging until monospaced=0
            fprintf(stderr,
                    " L%02d DBG1 x %3d %+4d %3d %+4d  d %3d %3d"
                    "  em %2d %2d  ex %2d\n# ...",
                    l1, pre1->x0, pre1->x1 - pre1->x0 + 1, box2->x0,
                    box2->x1 - box2->x0 + 1, d1, d2, mono_em_min, mono_em_max,
                    char_width_max);
#endif
      }
      ...
      if (pre2) {
        ...
        if ((48 & job->cfg.verbose) == 48)
          if (monospaced && l1)  // debugging until monospaced=0
            fprintf(stderr,
                    " L%02d DBG2 x %3d %+4d %3d %+4d  d %3d %3d"
                    "  em %2d %2d  ex %2d\n# ...",
                    l1, pre2->x0, pre2->x1 - pre2->x0 + 1, box2->x0,
                    box2->x1 - box2->x0 + 1, d1, d2, mono_em_min, mono_em_max,
                    char_width_max);
#endif
      }
...
// the upper part does good work, we do not need this stuff ... ???
      ...
      if (monospaced && pre1) {     // check 2 chars for non mono space within
        ...
        if (mono_em_max < 2 * mono_em_min &&
          ...
          if ((box2->x0 - pre1->x1 <=
                   mono_em_max  // no space between
                                //        && box2->x1 - pre1->x0 >
                                //        2*mono_em_max) // crossing 1 em
                                //        border?
               && box2->x1 - pre1->x0 >
                      2 * mono_em_min + mono_em_min / 8)  // 2018-09 rnd80
              || (box2->x0 - pre1->x1 > mono_em_min       // space between
                  && box2->x0 - pre1->x1 <= 2 * mono_em_max - mono_em_max / 16
                  //        && box2->x1 - pre1->x0 >  3*mono_em_max)) { //
                  //        crossing 2 em border?
                  && box2->x1 - pre1->x0 >
                         3 * mono_em_min + mono_em_min / 8)) {  // 2018-09 rnd80
            monospaced = 0;  // can not be monospaced in that case 2010-09-25
            if (job->cfg.verbose)
              fprintf(stderr,
                      " L%02d mono:=0  %d - %d  pre1 %d %d  %d %d y %d "
                      "DBG%d\n# ...",
                      l1, mono_em_min, mono_em_max, pre1->x0, pre1->x1,
                      box2->x0, box2->x1, box2->y0, __LINE__);
          }
      }
      ...
      if (monospaced && pre2 &&
          (2 * 2 + 2) * mono_em_max <
              (2 * 2 + 3) *
                  mono_em_min) {    // check 2 chars for non mono space within
        d1l = box2->x0 - pre2->x0;  // left to left distance
        d1r = box2->x1 - pre2->x1;  // right to right distance
        if (d1l > d1r) {
          d1 = d1r;
        ...
        if ((box2->x0 - pre2->x1 >
                 mono_em_min + mono_em_min / 16  // min. 1 char between
             && box2->x0 - pre2->x1 <=
                    2 * mono_em_max - mono_em_max / 8  // max. 2 chars
             //        && box2->x1 - pre2->x0 >  3*mono_em_max) // crossing 2 em
             //        border?
             && box2->x1 - pre2->x0 >
                    3 * mono_em_min + mono_em_min / 8)  // 2018-09 rnd80
            ||
            0 * (box2->x0 - pre2->x1 >
                     2 * mono_em_min + mono_em_min / 8  // min. 2 ex between
                 && box2->x0 - pre2->x1 <=
                        3 * mono_em_max - mono_em_max / 4  // ?????? ToDo oebb
                 //        && box2->x1 - pre2->x0 >  4*mono_em_max)) { //
                 //        crossing 3 em border?
                 && box2->x1 - pre2->x0 >
                        4 * mono_em_min + mono_em_min / 2)) {  // 2018-09 rnd80
          monospaced = 0;  // can not be monospaced in that case 2010-09-25
          if (job->cfg.verbose)
            fprintf(stderr,
                    " L%02d mono:=0  %d - %d  pre2 %d %d  %d %d DBG%d\n# ...",
                    l1, mono_em_min, mono_em_max, pre2->x0, pre2->x1, box2->x0,
                    box2->x1, __LINE__);
        }
      }
    ...
    if (job->cfg.verbose)
      fprintf(stderr,
              " L%02d num_gaps= %2d x_width= %2d - %2d"
              " mono_em= %2d - %2d  mono= %d",
              l1, numdists, char_width_min, char_width_max, mono_em_min,
              mono_em_max, monospaced);
    if (numdists < 8) {
    ...
    if (numdists > 0) {
      ...
      if (mono_em_min > mono_em_max + (mono_em_min + 4) / 9 + 1  // rnd80 52 45
        ...
        monospaced = 0;
        if (job->cfg.verbose)
          fprintf(stderr, "\n# ... L%02d mono:=0  %d - %d DBG%d", l1,
                  mono_em_min, mono_em_max, __LINE__);
      } else
        pitch_m = ((mono_em_max < 3 * mono_em_min)
                       ? (mono_em_max + 3 * mono_em_min) / 4
                       : mono_em_min);
      ...
      for (ni = ni_min = 1024, max = 0,
          i = ((numdists < 8) ? 0 : numdists / 2 + 1);
           i < numdists; i++) {
        if (pdists[i] <= char_width_min / 3) continue;  // JS-2010-09
        if (pdists[i] > char_width_max * 2) {
          /* set 2nd best which is numdists as default */;
          break;
        }                   // JS-2010-27 table gaps
        ...
        if (diff > max) {
          ...
          if ((job->cfg.verbose & (32 + 16)) == 48)
            fprintf(stderr, " L%02d best_p= %3d + maxdiff=%3d\n# ...", l1,
                    pdists[best_p], max);
          if (max > 3 && 3 * pdists[i] >= 4 * pdists[i - 1]) {
            break;
          }
          if (max > 1 && 3 * i > numdists * 2 &&
              3 * pdists[i] >= 4 * pdists[i - 1]) {
            break;
          }
        }
        if (diff) {
      ...
      if (numdists < 16 && max <= 1 && ni_min > 1)
        best_p = numdists - 1;  // one word
#if 1                           /* debugging */
      if ((job->cfg.verbose & (32 + 16)) == 48) {
        // fprintf(stderr,"\n# ...");
        for (i = 0; i < numdists; i++) fprintf(stderr, " %2d", pdists[i]);
      ...
      if (best_p < numdists - 1)
        pitch_p = ((pdists[best_p] + pdists[best_p + 1]) / 2 + 1);
      else
        pitch_p = (pdists[best_p] + 1);
      if (numdists)
      ...
      if (job->cfg.verbose) {
        ...
        fprintf(stderr,
                " L%02d prop: num=%3d min=%3d max=%3d pitch=%3d @ %2d%%\n# ...",
                l1, numdists, pdists[0], pdists[numdists - 1], pitch_p,
                best_p * 100 / numdists);
        fprintf(
            stderr,
            " L%02d result: mono=%d  distance >= %d considered as space\n# ...",
  ...
  if (job->cfg.spc == 0) job->cfg.spc = spc;
  if (job->cfg.verbose)
 ...
 */
int count_subboxes(pix *pp, job_t *job) {
  int ii = 0, num_mini = 0, num_same = 0, cnt = 0;
  struct box *box2, *box4;
  if (job->cfg.verbose) {
    fprintf(stderr, "# count subboxes\n# ...");
  }

  for_each_data(&(job->res.boxlist)) {
    box2 = (struct box *)list_get_current(&(job->res.boxlist));
    box2->num_subboxes = 0;
    if ((box2->x1 - box2->x0) < 2 || (box2->y1 - box2->y0) < 2)
      continue; /* speedup for dotted bg */
    ...
    for_each_data(&(job->res.boxlist)) {
          ...
          box4->num_subboxes == 0) /* box4 inside box2? */
      {
        box2->num_subboxes++;
        ii++;
        if ((box4->x1 - box4->x0 + 1) * (box4->y1 - box4->y0 + 1) < 17)
          num_mini++;
      }
    }
    end_for_each(&(job->res.boxlist));
  ...
  if (job->cfg.verbose)
    fprintf(stderr, " %3d subboxes counted (mini=%d, same=%d) nC= %d\n", ii,
            num_mini, num_same / 2 /* counted twice */, cnt);
  return 0;
}
...
*/
int glue_holes_inside_chars(pix *pp, job_t *job) {
  int ii, x0, y0, x1, y1, cnt = 0, glued_same = 0, glued_holes = 0;
  struct box *box2, *box4;
  // int cs=job->cfg.cs;
  {
    count_subboxes(pp, job); /* move to pgm2asc() later */
      ...
      for_each_data(&(job->res.boxlist)) {
        ...
        if (box4 != box2 && box4->c != PICTURE) {
          ...
          if (abs(box4->frame_vol[0]) >=
            ...
            if ((box4->x0 == x0 && box4->x1 == x1 && box4->y0 == y0 &&
                 box4->y1 == y1) /* do not happen !? */
                || (box4->x0 >= x0 && box4->x1 <= x1 && box4->y0 >= y0 &&
                    box4->y1 <= y1
...
/* ---- join broken chars ( before step1 ??? )  -----------------------
    ...
    ToDo: store m4 of upper line to m4_of_prev_line, and check that "-points are
   below done: join boxes lying inside another box (usually holes, ex: "aeobdg")
...
int glue_broken_chars(job_t *job, pix *pp) {
  int ii, y, cs, x0, y0, x1, y1,
      cnt = 0, num_frags = 0, glued_frags = 0, glued_hor = 0,
    ...
    for_each_data(&(job->res.boxlist)) {
      ...
      if (2 * (y1 - y0) < box2->m4 - box2->m1         // care for dots etc.
        ...
        for_each_data(&(job->res.boxlist)) {
          ...
          if (box4->line >= 0 && box2->line >= 0 && box4->line == box2->line) {
            ...
            if (abs(box4->x0 + box4->x1 - 2 * box2->x0) <
                abs(box5->x0 + box5->x1 -
                    2 * box2->x0)) { /* box6=box5; next-nearest box */
              box5 = box4;
            }
          }
        }
        ...
        if (box4) {
          ...
          if (box4->x1 - box4->x0 + 1 > job->res.avX / 2 &&
              ...
              (box2->x0 > box4->x1 || box4->x0 > box2->x1))
            continue;
          ...
          if (do_join > 0) {           // fkt melt(box2,box4)
            ...
            merge_boxes(box2, box4, job);  // add box4 to box2
            x0 = box2->x0;
            x1 = box2->x1;
            y0 = box2->y0;
            y1 = box2->y1;
          ...
          for_each_data(&(job->res.boxlist)) {
            ...
            if (box4 != box2 && box4->c != PICTURE) {
              if (box4->line >= 0 && box4->line == box2->line &&
                ...
                if (get_bw(x0, x0, y1, y1, pp, cs, 1, job) == 1)
                  if (get_bw(x0 - 2, x0 - 1, y1, y1 + 2, pp, cs, 1, job) ==
                      1) {  // fkt melt(box2,box4)
                    if (job->cfg.verbose & 7)
                      fprintf(stderr,
                              " join objects  %4d %4d %+4d %+4d"
                              " + %4d %4d %+4d %+4d w'K'\n# ...",
                              x0, y0, x1 - x0 + 1, y1 - y0 + 1, box4->x0,
                              box4->y0, box4->x1 - box4->x0 + 1,
                              box4->y1 - box4->y0 + 1);
                    put(pp, x0, y1 + 1, ~(128 + 64), 0);
                    merge_boxes(box2, box4, job);
                    x0 = box2->x0;
                    x1 = box2->x1;
                    y0 = box2->y0;
                    y1 = box2->y1;
      ...
      if (abs(box2->m2 - y0) <= (y1 - y0) / 8)
        if (abs(box2->m3 - y1) <= (y1 - y0) / 8)
          if (num_cross(x0, x1, (y0 + y1) / 2, (y0 + y1) / 2, pp, cs, job) == 1)
            if (num_cross(x0, x1, (y0 + 3 * y1) / 4, (y0 + 3 * y1) / 4, pp, cs,
                          job) == 1)
              if (get_bw((3 * x0 + x1) / 4, (3 * x0 + x1) / 4,
                         (3 * y0 + y1) / 4, y1, pp, cs, 1, job) == 0)
                if (get_bw(x0, (3 * x0 + x1) / 4, (3 * y0 + y1) / 4,
                           (y0 + 3 * y1) / 4, pp, cs, 1, job) == 0)
                  if (get_bw(x0, x0, y0, (3 * y0 + y1) / 4, pp, cs, 1, job) ==
                      1)
                    for_each_data(&(job->res.boxlist)) {
                      box4 =
                          (struct box *)list_get_current(&(job->res.boxlist));
                      if (box4 != box2 && box4->c != PICTURE) {
                        if (box4->line >= 0 && box4->line == box2->line &&
                            box4->x1 > x0 - 3 && box4->x1 - 2 < x0 &&
                            abs(box4->y1 - box2->m3) <
                                2) {  // fkt melt(box2,box4)
                          if (job->cfg.verbose & 7)
                            fprintf(stderr,
                                    " join objects %4d %4d %+4d %+4d"
                                    " + %4d %4d %+4d %+4d nh\n# ...",
                                    ...
                                    box4->y0, box4->x1 - box4->x0 + 1,
                                    box4->y1 - box4->y0 + 1);
                          y = loop(pp, x0, y0, y1 - y0, cs, 0, DO, job);
                          if (2 * y > y1 - y0) continue;
...
int find_same_chars(pix *pp, job_t *job) {
  int i, k, d, cs, dist, n1, dx;
  struct box *box2, *box3, /* *box4, */ *box5;
  pix p = (*pp);
  cs = job->cfg.cs;
  {
      ...
      if (dx > 3)
        for (box3 = (struct box *)list_next(&(job->res.boxlist), box2); box3;
          ...
          if (box2->num != box3->num) {
            int d = distance(&p, box2, &p, box3, cs, job);
            if (d < dist) {
              dist = d;   /* box4=box3; */
            }             // best fit
            if (d < 5) {  // good limit = 5% ???
              i--;
              n1 = box3->num;  // set all num==box2.num to box2.num
              for_each_data(&(job->res.boxlist)) {
                box5 = (struct box *)(struct box *)list_get_current(
                    &(job->res.boxlist));
                if (box5 != box2)
    ...
    for_each_data(&(job->res.boxlist)) {
      ...
      for (box3 = box2, j = 0; box3;
        ...
        if (box3->num == box2->num) {
          j++;
          d = distance(&p, box2, &p, box3, cs, job);
          if (d > dist) {
            dist = d;
            box4 = box3;
          }  // worst fit
...
**
*/
int char_recognition(pix *pp, int mo, job_t *job) {
  int i, ii, ni, cs, x0, y0, x1, y1;
  struct box *box2;
  wchar_t cc;
  cs = job->cfg.cs;
  // ---- analyse boxes, find chars ---------------------------------
  if (job->cfg.verbose) fprintf(stderr, "# char recognition");
  i = ii = ni = 0;
  ...
  for_each_data(&(job->res.boxlist)) {
    ...
    if ((mo & 256) == 0) { /* this case should be default (main engine) */
      if (cc == UNKNOWN || box2->num_ac == 0 ||
          box2->wac[0] < job->cfg.certainty)
        cc = whatletter(box2, cs, 0, job);
    }
    ...
    if (job->cfg.verbose & 8) {
      fprintf(stderr, "\n# code= %04lx %c", (long)cc,
              (char)((cc < 255) ? cc : '_'));
      out_b(box2, pp, x0, y0, x1 - x0 + 1, y1 - y0 + 1, cs, job);
    }
  }
  end_for_each(&(job->res.boxlist));
  if (job->cfg.verbose)
    fprintf(stderr, " %d of %d chars unidentified\n", i, ii);
  return 0;
}
...
** improve the result
*/
int compare_unknown_with_known_chars(pix *pp, int mo, job_t *job) {
  ...
  if (!(mo & 8)) {
    ii = ni = 0;
    for_each_data(&(job->res.boxlist)) { ni++; }
    end_for_each(&(job->res.boxlist));
    for_each_data(&(job->res.boxlist)) {
      box2 = (struct box *)list_get_current(&(job->res.boxlist));
      ii++;
      if (box2->c == UNKNOWN || (box2->num_ac > 0 && box2->wac[0] < 97))
        if (box2->y1 - box2->y0 > 4 && box2->x1 - box2->x0 > 1) {  // no dots!
          ...
          for_each_data(&(job->res.boxlist)) {
            ...
            d = distance(pp, box2, pp, box3, cs, job);
            if (d < dist) {
              dist = d;
              bc = box3->c;
              box4 = box3;
          ...
          if (dist < 50 && (job->cfg.verbose & 7)) {  // only for debugging
            fprintf(stderr,
                    ...
                    " dist=%3d%% i=%d",
                    box2->line, box2->x0, box2->y0, (int)bc,
                    (char)((bc < 128) ? bc : '_'), dist, i);
            if (box4->num_ac > 0) fprintf(stderr, " w= %3d%%", box4->wac[0]);
            if ((job->cfg.verbose & 4) && dist < 10) out_x2(box2, box4, job);
          }
        }
    }
    end_for_each(&(job->res.boxlist));
  }
  if (job->cfg.verbose) fprintf(stderr, " - found %d (nC=%d)\n", i, ii);
  return 0;
}
...
*/
int try_to_divide_boxes(pix *pp, int mo, job_t *job) {
  struct box *box2, boxa, boxb;
  int cs = job->cfg.cs, ad = 100, a2[8],
      ar,         // certainty of each part, ar = product of all certainties
  ...
  if (!(mo & 16))  // put this to the caller
    for_each_data(&(job->res.boxlist)) {
      ...
      if ((!box2->num_frames) ||
          box2->num_frame_vectors[box2->num_frames - 1] < 9)
        continue;
      if ((box2->c == UNKNOWN ||
           (box2->num_ac && box2->wac[0] < job->cfg.certainty)) &&
        ...
        /* ocr1809_12minus "-5" */
        ii = loop(pp, x0 + 1, y0, y1 - y0, cs, 0, DO, job);
        i = loop(pp, x0 + 1, y1, y1 - y0, cs, 0, UP, job);
        if (ii + i >= 7 * (y1 - y0 - 1) / 8 && y0 + ii > box2->m2 &&
            y0 + ii < box2->m3) {                   // check for "-5"
          for (i1 = 0; i1 < (x1 - x0) / 2; i1++) {  // check v-symmetry
            i2 = loop(pp, x0 + i1, y0, y1 - y0, cs, 0, DO, job);
            if (abs(i2 - ii) > (y1 - y0) / 16) break;  // not or end of -
            i2 = loop(pp, x0 + i1, y1, y1 - y0, cs, 0, UP, job);
            if (abs(i2 - i) > (y1 - y0) / 16) break;  // not or end of -
          }
          if ((job->cfg.verbose & 2) /* && i1>(x1-x0)/3*/) {
            fprintf(stderr,
                    "\n# try_to_divide_box(xy,dxy): %4d %4d %3d %3d as -5 "
                    "xcut= %d-1",
          ...
          if (i1 > (x1 - x0 - 1) / 4) {
            ...
            boxa.x = x0;
            boxa.y = y0;  // obsolete? mark pixel, overlap?
            boxa.x0 = xi[i] = x0;
            boxa.x1 = xi[i + 1] = x0 + i1 - 1;  // new horizontal box range
            cut_box(&boxa, job);
            boxa.num_ac = 0;  // ToDo: add box2 as src argument?
            ci[i] = whatletter(&boxa, cs, 0, job); /* get char */
            a2[i] = testac(&boxa, ci[i]);          /* get certainty */
            if ((ci[i] == '-' || ci[i] == '_') && a2[i] >= 97)  // 2018-09 "-5"
            {
              setac(&boxa, ci[i], a2[i] = 99, job);
            ...
            boxb.y = y0;
            boxb.x0 = xi[i] + 1;
            boxb.x1 = xi[i + 1] = box2->x1;
            cut_box(&boxb, job);
            boxb.num_ac = 0;
            ci[i] = whatletter(&boxb, cs, 0, job);
            a2[i] = testac(&boxb, ci[i]);
            if (a2[0] >= 97 && a2[1] >= 99)  // 2018-09 "-5"
            {
              char buf[8] = "";
              setac(&boxb, ci[i], a2[i] = 99, job);
              if ((job->cfg.verbose & 2)) {
                DBG(fprintf(stderr, "\nDBG %s set split certainty 99",
                            decode(ci[1], ASCII)));
              }
              ...
              if (buf[0]) setas(box2, buf, ar, job);
            }
          }
        } /* check "-5" split */

        /* get minimum vertical lines, but fails on ocr1809_12minus "-5" */
        n1 = num_cross(x0, x1, (y1 + y0) / 2, (y1 + y0) / 2, pp, cs, job);
        ...
        if (box2->m2 && box2->m3 > box2->m2 + 2)
          for (i = box2->m2 + 1; i <= box2->m3 - 1; i++) {
            ...
            if (loop(pp, x0 + 1, i, x1 - x0, cs, 1, RI, job) > (x1 - x0 - 2))
              continue;  // ll
            ii = num_cross(x0, x1, i, i, pp, cs, job);
            if (ii < n1) n1 = ii;
          }
        if (n1 < 2) continue;  // seems to make no sense to divide
        if (n1 < 4)
          ad = 99 * ad / 100;  // not to strong because m2+m3 could be wrong
        if (n1 < 3) ad = 99 * ad / 100;

        if (2 * y1 < box2->m3 + box2->m4 /* baseline char ? */
            && num_cross(x0, x1, y1 - 1, y1 - 1, pp, cs, job) ==
                   1  // -1 for slopes
            && num_cross((x0 + 2 * x1) / 3, (x0 + 3 * x1) / 4, y0, y1, pp, cs,
                         job) < 3  // not exclude tz
            && num_cross((3 * x0 + x1) / 4, (2 * x0 + x1) / 3, y0, y1, pp, cs,
                         job) < 3  // not exclude zl
            &&
            loop(pp, x0, y1 - (y1 - y0) / 32, x1 - x0, cs, 0, RI, job) +
                    loop(pp, x1, y1 - (y1 - y0) / 32, x1 - x0, cs, 0, LE, job) >
                (x1 - x0 + 1) / 2)
          continue; /* do not try on bvdo"o etc. */

        // one vertical line can not be two glued chars, lc?
        if (num_cross(x0, x1, (y1 + y0) / 2, (y1 + y0) / 2, pp, cs, job) <= 1)
          continue;
        {  // doublet = 2 letters
          ...
          if (job->cfg.verbose & 2) {
            ...
            fprintf(
                ...
                x0, y0, x1 - x0 + 1, y1 - y0 + 1, l1,
                job->res.lines.mono[l1] /* 1=mono */);
            if (job->cfg.verbose & 4)
              out_x(box2, job);  // list box as data+ASC-image
          }
              ...
              if (box2->frame_vector[i4][0] != x0 &&
                ...
                for (i5 = i4 + 2; i5 < num_allvec; i5++)
                  ...
                  if (box2->frame_vector[i4][0] == box2->frame_vector[i5][0] &&
                    ...
                    if (job->cfg.verbose & 2)
                      fprintf(stderr,
                              "DBG vsplit i45= %d %d i67 %d %d i89 %d %d"
                              " xy=%2d %2d ToDo\n",
                              i4, i5, i6, i7, i8, i9,
          ...
          xi[0] = x0;
          xi[1] = x0 + (dx / 8) + 1;
          xi[2] = x1;             // split_to xi0..1 and xi1..2
          for (;; xi[i + 1]++) {  // x[i] .. x[i+1], slower? but better v0.42
            ...
            if (mono &&  // 2018-10 use monofont advantage
                (abs(abs(xi[i + 1] - x0) -
                     job->res.lines.pitch[l1]) /* 2 chars */
                     > job->res.lines.pitch[l1] / 8 &&
                 abs(abs(xi[i + 1] - x0) -
                     2 * job->res.lines.pitch[l1]) /* 3 chars */
                     > job->res.lines.pitch[l1] / 8))
              continue;
            if (mono &&
                job->cfg.verbose & 2)  // rnd80-Droid-Sans-Mono-Regular ww
              fprintf(stderr, "\n#DBG monosplit x01,xi,pitch= %4d %4d %4d %4d",
                      x0, x1 - x0 + 1, xi[i + 1] - x0,
            ...
            //          i1=nearest_frame_vector(box2, 0,num_allvec,
            //          (xi[0]+3*xi[1])/4, y1); i3=nearest_frame_vector(box2,
            //          0,num_allvec, (3*xi[1]+xi[2])/4, y1);
            i1 = nearest_frame_vector(box2, 0, num_allvec, (xi[0] + xi[1]) / 2,
                                      y1, job);
            i3 = nearest_frame_vector(box2, 0, num_allvec, (xi[1] + xi[2]) / 2,
                                      y1, job);
            i2 = nearest_frame_vector(box2, i1, i3, xi[1], y0, job);
            ...
            if (job->cfg.verbose & 2)
              fprintf(stderr,
                      ...
                      " bow %d i123=%2d %2d %2d",
                      i, xi[i] - x0, xi[i + 1] - x0, xi[i + 2] - x0, bow, i1,
                      i2, i3);
            ...
            // removing ->dots if dot only above one char !!! ??? not
            // implemented
            if (job->cfg.verbose & 2)
              fprintf(stderr,
            ...
            boxa.y = y0;  // obsolete? mark pixel, overlap?
            boxa.x0 = xi[i];
            boxa.x1 = xi[i + 1];  // new horizontal box range
            ...
            a2[i] = testac(&boxa, ci[i]);          /* get certainty */
            if ((ci[i] == 'c' || ci[i] == 'C') &&
                a2[i] == 100)  // 2018-09 "ow" read as 100% "cw"
            {
              setac(&boxa, ci[i], a2[i] = 99, job);
              DBG(fprintf(stderr, "\nDBG set split certainty 99"));
            }
            if (job->cfg.verbose & 2)
              fprintf(stderr, "\n#  certainty %d  limit= %d  cbest= %d ", a2[i],
                      job->cfg.certainty, cbest);
            ...
            if (ar < 98 * job->cfg.certainty / 100 || ar < cbest) {
              continue;
            }  // dont go deeper, no longer string

            i++;
            if (i == 8) break;  // maximum splits
            if (i == 4) break;  // at the moment its to slow to go further
            if (i + 1 < 8) xi[i + 1] = x1;  // right border of next box
            if (i + 2 < 8) xi[i + 2] = x1;

            if (job->cfg.verbose & 2)
              fprintf(stderr, "\n try end split [%d].x=%d [%d].x=%d ", i,
                      xi[i] - x0, i + 1, xi[i + 1] - x0);
            boxb = *box2;  // try rest if it has to be split again
            boxb.x = xi[i] + 1;
            boxb.y = y0;
            boxb.x0 = xi[i] + 1;
            ...
            setas(box2, buf, ar,
                  job);  // char *, does it disturb further splitting?
            buf[0] = 0;
            i--;
            xi[i + 2] = x1;
          } /* xi[i+1]++ */
...
*/
int divide_vert_glued_boxes(pix *pp, int mo, job_t *job) {
  struct box *box2, *box3, *box4;
  ...
  for_each_data(&(job->res.boxlist)) {
    ...
    y1 = box2->y1;
    dy = y1 - y0 + 1;
    dx = 4 * (job->res.avX + box2->x1 - box2->x0 +
              1);  // we want to be sure to look at 4ex distance
    if (dy > 2 * job->res.avY && dy < 6 * job->res.avY && box2->m1 &&
        y0 <= box2->m2 + 2 && y0 >= box2->m1 - 2 &&
        y1 >= box2->m4 + job->res.avY -
                  2) {  // test if lower end fits one of the other lines?
      box4 = box2;
      flag_found = 0;
      for_each_data(&(job->res.boxlist)) {
        box4 = (struct box *)list_get_current(&(job->res.boxlist));
        if (box4->c != UNKNOWN) continue; /* dont try on pictures */
        if (box4->x1 < box2->x0 - dx || box4->x0 > box2->x1 + dx)
          continue;  // ignore far boxes
        if (box4->line == box2->line)
          flag_found |= 1;  // near char on same line
        if (box4->line == box2->line + 1)
          flag_found |= 2;           // near char on next line
        if (flag_found == 3) break;  // we have two vertical glued chars
      }
      end_for_each(&(job->res.boxlist));
      if (flag_found != 3)
        continue;    // do not divide big chars or special symbols
      y = box2->m4;  // lower end of the next line
      if (job->cfg.verbose & 2) {
        fprintf(stderr, "\n# divide box below y=%4d", y - y0);
        if (job->cfg.verbose & 6) out_x(box2, job);
      }
      // --- insert box3 before box2
      box3 = (struct box *)malloc_box(box2);
      box3->y1 = y;
      box2->y0 = y + 1;
      box2->line++;  // m1..m4 should be corrected!
      if (box4->line == box2->line) {
        box2->m1 = box4->m1;
        box2->m2 = box4->m2;
        box2->m3 = box4->m3;
...
int setc(struct box *box2, wchar_t cc, job_t *job) {
  int ret = 0, w2;  // w1
  ...
  if (job->cfg.verbose) {
    ...
    fprintf(stderr, "\n#  setc old nac=%d %s %s %3d %3d  to %s %3d at %4d %4d",
            ...
            (box2->num_ac < 2) ? " " : decode(box2->tac[1], ASCII),
            box2->wac[0], (box2->num_ac < 2) ? 0 : box2->wac[1],
            decode(cc, ASCII), (100 + w2 + 1) / 2, box2->x0, box2->y0);
  }
  if (w2) {
    if (box2->c != cc) {
      ret = 1;
      setac(box2, cc, (100 + w2 + 1) / 2, job);
    }
  }
...
int context_correction(job_t *job) {
  ...
  int dx, dy, O0_num = 0, O0_slashed_zeros = 0, O0_maxw = 0, O0_minw = 999999,
              O0_maxh = 0, O0_minh = 999999;
  //  pix *pp = &(job->src.p);
  ...
  if (job->cfg.verbose) fprintf(stderr, "# context correction Il1 O0\n");
  ...
  end_for_each(&(job->res.boxlist));
  if (job->cfg.verbose)
    fprintf(stderr, "# O0 num= %d slashed0=%d mimaxW %d %d", O0_num,
            O0_slashed_zeros, O0_minw, O0_maxw);

  // 2nd loop to make corrections
  for_each_data(&(job->res.boxlist)) {
    pre4 = pre3;
    pre3 = pre2;
    pre2 = prev;  // 2010-10-01 tmp08/080916_JL*_150
    box2 = (struct box *)list_get_current(&(job->res.boxlist));
    ...
    if (box2->c && strchr("O0lI123456789ABCDEFabcdef", box2->c))
      hexdigits++;
    ...
    if (box2->c && strchr("O0", box2->c) && hexdigits > 5)
      nc += setc(box2, (wchar_t)'0', job);
    if (box2->c && strchr("l1", box2->c) && hexdigits > 5)
      nc += setc(box2, (wchar_t)'1', job);
    ...
    if (box2->c == QUOTATION_MARK  // 0x22 = ""
      ...
      if (job->cfg.verbose & 4)
        fprintf(stderr, "\n#  change nac=%d %s   %3d to %s %3d at %3d %3d",
                box2->num_ac, "\"", box2->wac[0], decode(box2->c, ASCII),
                box2->wac[0], box2->x0, box2->y0);
    ...
    if (box2->c && strchr("Il1|", box2->c) && next && prev) {
      ...
      if (wisalpha(next->c) && next->c != 'i' &&
        ...
        nc += setc(box2, (wchar_t)'I', job);
      } else if ((box2->c != '1' /* lnt => Int, but 1st */
                  && strchr(l_nonvo, next->c) && strchr("\" \n", prev->c)) ||
                 (prev &&
                  ((!pre2) || wisupper(pre2->c) || strchr(" \n", pre2->c)) &&
    ...
    /* check for O0 */
      ...
      for (i0 = 0; i0 < 5;
           i0++) { /* ToDo: take into account 100% chars only? */
        c0 = '\0';
      ...
      if ((have_hexlo && have_hexhi) || have_alpha || have_upper) {
        have_upper += have_hexhi;
        have_alpha += have_hexlo;
        have_hexlo = 0;
        have_hexhi = 0;
      }  // have_hex*=0 if isalpha
      ...
      if (O0_slashed_zeros == 0 && O0_num > 1  // 2018-09 rnd80.tt
          ...
          dy >= box2->m3 - box2->m2) {
        nc += setc(box2, (wchar_t)'O', job);  // big width
        if (job->cfg.verbose & 4)
          fprintf(stderr, " DBG%04d %d,%d: O0 to O", __LINE__, box2->x0,
                  box2->y0);
      } else if (O0_slashed_zeros == 0 && O0_num > 1  // 2018-09 rnd80.tt
                 && O0_maxw > O0_minw + (box2->x1 - box2->x0 + 1) / 32 + 1 &&
                 (box2->x1 - box2->x0 + 1) < (O0_maxw + O0_minw + 1) / 2 &&
                 dy >= box2->m3 - box2->m2) {
        nc += setc(box2, (wchar_t)'0', job);  // small width
        if (job->cfg.verbose & 4)
          fprintf(stderr, " DBG%04d %d,%d: O0 to 0", __LINE__, box2->x0,
                  box2->y0);
    ...
    if (wisupper(box2->c) && next && prev) {
      if (wislower(prev->c) && wislower(next->c) &&
        ...
        box3->c = ' ';
        box3->modifier = 0;
        setac(box3, ' ', 99, job); /* ToDo: weight depends from distance */
        box3->num = -1;
        box3->line = prev->line;
...
int list_insert_spaces(pix *pp, job_t *job) {
  int i = 0, j1, j2, i1, maxline = -1, dy = 0, num_nl = 0, num_spc = 0,
      min_x0 = 1023;
  char cc;
  ...
  for (i1 = 1; i1 < job->res.lines.num; i1++) {
    ...
    if (j1 > dy * 120 / 100 || j1 < dy * 80 / 100)
      continue;  // only most frequently
    j2 += j1;
    i++;
  }
  ...
  for_each_data(&(job->res.boxlist)) {
    ...
    if (box2->line > maxline) {  // new line, lines and chars must be sorted!
      ...
      if (maxline >= 0) {
        ...
        ypitch = job->res.lines.m4[box2->line] - job->res.lines.m1[box2->line];
    ...
    if (box2->line == maxline) {  // lines and chars must be sorted!
      ...
      if (num_nl || !box3) pdist = box2->x0 - min_x0;  // first char of new line
      // if (pdist >= thispitch) cc=' '; // 2010-09-24 ???
      if (thismono)
        num_spc = pdist / thispitch;
      else
        num_spc = pdist * 2 / (3 * job->res.avX);       // ToDo: use 1em!
    ...
    for (i1 = 0; i1 < num_nl + num_spc; i1++) {
      ...
      if (box4)
        mdist = box2->x0 - box4->x1 + 1;  // 2010-09
      else
        mdist = 0;
      if (mdist < 0) mdist = 0;
      box3 = (struct box *)malloc_box(NULL);
      ...
      box3->x1 =
          box2->x0 - 2 + ((num_spc) ? -mdist + (i1 + 1) * mdist / num_spc : 0);
      box3->y0 = box2->y0;
      box3->y1 = box2->y1;
      if (i1 >= num_nl && box4)
        box3->x0 = box4->x1 + 2 + ((num_spc) ? i1 * mdist / num_spc : 0);
      if (i1 < num_nl || !box4) box3->x0 = job->res.lines.x0[box2->line];
      if (i1 < num_nl && box4) {
      ...
      box3->m4 = box2->m4;
      box3->p = pp;
      setac(box3, cc, 100, job); /* ToDo: weight depends from distance */
      list_ins(&(job->res.boxlist), box2, box3);  // insert box3 before box2
      if (job->cfg.verbose & 1) {
        fprintf(stderr,
                "\n# insert space &%d; at %4d %4d box= %p"
                " mono %d dx %2d pdx,mdx %2d %2d",
                (int)box3->c, box3->x0, box3->y0, (void *)box3, thismono,
                thispitch, pdist, mdist);
        /* out_x(box3); */
      }
      i++;
    }
...
int add_line_info(job_t *job /* , List *boxlist2 */) {
  ...
  for_each_data(&(job->res.boxlist)) {
    ...
    {  // add rotated image correction dy(x)
      if (lines->dx)
        xx = lines->dy * ((box2->x1 + box2->x0) / 2) / lines->dx;
      else
        xx = 0;
      m1 = lines->m1[i] + xx;
      m2 = lines->m2[i] + xx;
      ...
      if (mindy1 > abs(box2->y1 - m4)) mindy1 = abs(box2->y1 - m4);
      mindy2 = 999999;
      if (box2->m2) {
        mindy2 = abs(box2->y0 - box2->m1);
      ...
      if (((box2->y1 - box2->y0 + 1) <= (m3 - m2 + 1) / 2 &&
           ...
           box2->y1 - job->res.avY / 2 - 2 <=
               m4)  // dots (ToDo: better 2nd run)
        ...
        if (box2->x0 >= lines->x0[i] && box2->x1 <= lines->x1[i] + job->res.avX)
          if (box2->y0 <= m4 + 2 * job->res.avY  // 2010-10-01+09 0811qemu2
            ...
            if (box2->m2 ==
                    0  // already put to a line? check y-distance
                       // || abs(box2->y0 - box2->m2) > abs(box2->y0 - m2)
                || mindy1 < mindy2) { /* found nearest line */
              if ((job->cfg.verbose & 16) &&
                  (box2->y1 - box2->y0 + 1) <= (m3 - m2 + 1) / 2)
                fprintf(
                    stderr,
    ...
    if (((box2->y1 - box2->y0 + 1) >=
         (box2->m3 - box2->m2 + 1) / 2)  // body not dots
        && (box2->y1 + 2 < box2->m1 ||
            box2->y0 < box2->m1 - (box2->m3 - box2->m1) / 2 ||
            box2->y0 - 2 >
                box2->m4 + (box2->m3 - box2->m2) / 2  // bad m4 + ,._ ocr-b
            || box2->y1 > box2->m3 + (box2->m3 - box2->m1))) /* to far away */
    {                                                        /* reset */
      if (job->cfg.verbose & 16)
        fprintf(stderr,
                "\n#  line.info.reset L%02d xy= %4d %4d m14 %4d %4d avY %4d",
                box2->line, box2->x0, box2->y0, box2->m1, box2->m4,
                job->res.avY);
      box2->m1 = 0;
      box2->m2 = 0;
      box2->m3 = 0;
...
// output_list(job);  // for debugging
  ...
  glue_holes_inside_chars(pp, job); /* including count subboxes (holes)  */

  detect_rotation_angle(job);

#if 1 /* Rotate the whole picture! move boxes */
  if (job->res.lines.dy !=
      0) {  // move down lowest first, move up highest first
    // in work! ??? (at end set dy=0) think on ppo!
  }
#endif
  detect_text_lines(pp, job->cfg.mode, job); /* detect and mark job->tmp.ppo */
  ...
  divide_vert_glued_boxes(pp, job->cfg.mode,
                          job); /* after add_line_info, before list_sort! */
  ...
  if (job->cfg.mode & 64) find_same_chars(pp, job);
  //  if(job->cfg.verbose&32) debug_img("out16",job,4+8);

  char_recognition(pp, job->cfg.mode, job);
  //  if(job->cfg.verbose&32) debug_img("out17",job,4+8);

  if (adjust_text_lines(pp, job->cfg.mode, job)) { /* correct using chars */
...
#define BlownUpDrawing 1 /* german: Explosionszeichnung, temporarly */
  ...
  compare_unknown_with_known_chars(pp, job->cfg.mode, job);

  try_to_divide_boxes(pp, job->cfg.mode, job);
  ...
  // ---- proof difficult chars Il1 by context view ----
  if (job->cfg.verbose) fprintf(stderr, "# context correction if !(mode&32)\n");
 ...
 */
  ...
  {                     /* just for debugging */
    int i, ii, ni;
    struct box *box2;
    const char *testc = "0123456789ABCDEFGHIJK";
    i = ii = ni = 0;
    for_each_data(&(job->res.boxlist)) { /* count boxes */
      box2 = (struct box *)list_get_current(&(job->res.boxlist));
      if (box2->c == UNKNOWN) i++;
    ...
    for (i = 0; i < 20; i++) {
      ...
      end_for_each(&(job->res.boxlist));
      if (job->cfg.verbose && ni > 0) fprintf(stderr, " (%c)=%d", testc[i], ni);
    }
