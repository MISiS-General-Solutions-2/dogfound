...
#define MAXL 10
void ocr2(pix *b, int cs, job_t *job) {
    ...
    for (y1 = 0; y1 < b->y; y1++)  // brute force
      for (x2 = 0; x2 < b->x; x2++)
        for (y2 = y1 + 1; y2 < b->y; y2++) {
          if (get_line2(x1, y1, x2, y2, b, cs, 100, job) > 99) {  // line ???
            l = (x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1);    // len
            for (i = 0; i < MAXL; i++) {  // remove similar lines (same middle
                                          // point) IMPROVE IT !!!!!! ???
              if (abs(x1 + x2 - xa[i] - xb[i]) < 1 + b->x / 2 &&
                  abs(y1 + y2 - ya[i] - yb[i]) < 1 + b->y / 2 &&
                  abs(y1 - ya[i]) < 1 + b->y / 4 &&
                  abs(x1 - xa[i]) < 1 + b->x / 4) {
                if (l > ll[i]) {
                  for (j = i; j < MAXL - 1; j++) {  // shift table
                    xa[j] = xa[j + 1];
                    ya[j] = ya[j + 1];
                    xb[j] = xb[j + 1];
                    yb[j] = yb[j + 1];
