...
int mean_thickness(struct box *box2, job_t *job) {
  int mt = 0, i, y, dx = box2->x1 - box2->x0 + 1, dy, cs = job->cfg.cs;
  for (y = box2->y0 + 1; y < box2->y1; y++) {
    i = loop(box2->p, box2->x0 + 0, y, dx, cs, 0, RI, job);
    i = loop(box2->p, box2->x0 + i, y, dx, cs, 1, RI, job);
    mt += i;
  }
...
int remove_dust(job_t *job) {
  ...
  /* FIXME jb:remove pp */
  pix *pp = &job->src.p;
  int i1, i, j, x, y, x0, x1, y0, y1, nC, sX, sY, sP, cs,
      vvv = job->cfg.verbose;
  struct box *box2;
#define HISTSIZE 220 /* histogramm size */
  ...
  if (job->cfg.dust_size < 0 && nC > 0) { /* auto detection */
    ...
    if (sY / nC < 10)
      job->cfg.dust_size = 1;  // 2010-09-24 6x9 gocr0801_bad5_FP
    ...
    for (i = 1; i + 3 < HISTSIZE; i++) { /* dust size = frame_vol */
      ...
      if (i < job->cfg.dust_size / 16 && i < 9)
        continue; /* giant font? tmp10/invalid_olv 2010-10 */
      if (histo[i /*+1*/] == 0) break; /* bad statistic */
      if ((histo[i + 2] + histo[i + 3]) >= (histo[i] + histo[i + 1]))
        break; /* no noise, but to late? */
      if (histo[i - 1] > 1024 * histo[i] && 2 * histo[i + 1] >= histo[i])
        break; /* bg pattern */
    }
    if (vvv) fprintf(stderr, " break");
    if (vvv)
      for (i1 = 0, j = i + 1; j < HISTSIZE; j++) {
        /* compressed, output only if something is changing */
        if (j == HISTSIZE - 1 || histo[j] != histo[j - 1] ||
            histo[j] != histo[j + 1]) {
    ...
    if (vvv)
      fprintf(stderr,
              ...
              (job->res.sumY + job->res.numC / 2) / job->res.numC, sX / nC,
              sY / nC);
  }
  if (job->cfg.dust_size) {
    ...
    if (vvv) {
      ...
      fprintf(
          stderr, " histo=%d,%d(?=%d),%d(?=%d),...\n# ...", histo[1], histo[2],
          histo[1] * histo[1] / (pp->x * pp->y), histo[3],
          histo[1] * histo[1] / (pp->x * pp->y) * histo[1] / (pp->x * pp->y));
    }
    i = 0;
    for_each_data(&(job->res.boxlist)) {
      box2 = (struct box *)list_get_current(&(job->res.boxlist));
  ...
  for_each_data(&(job->res.boxlist)) {
    ...
    if (x1 - x0 > 16 && y1 - y0 > 30) /* only on large enough chars */
      for (x = x0 + 1; x <= x1 - 1; x++)
        for (y = y0 + 1; y <= y1 - 1; y++) {
          if (pixel_atp(pp, x, y) >= cs && pixel_atp(pp, x - 1, y) < cs &&
              pixel_atp(pp, x + 1, y) < cs && pixel_atp(pp, x, y - 1) < cs &&
...
int smooth_borders(job_t *job) {
  ...
  n1 = n2 = 0;
  if (vvv) {
    fprintf(stderr, "# smooth_borders of big chars 7x16 cs=%d", cs);
  }
  ...
  for_each_data(&(job->res.boxlist)) {
    ...
    if (mean_thickness(box2, job) < 3) continue;
    ...
    for (x = box2->x0; x <= box2->x1; x++)
      for (y = box2->y0; y <= box2->y1; y++) { /* filter out high frequencies */
        ...
        cn[0] = getpixel(pp, x - 1, y, job);
        cn[4] = getpixel(pp, x + 1, y, job); /* horizontal */
        cn[2] = getpixel(pp, x, y - 1, job);
        cn[6] = getpixel(pp, x, y + 1, job); /* vertical */
        cn[1] = getpixel(pp, x - 1, y - 1, job);
        cn[3] = getpixel(pp, x + 1, y - 1, job); /* diagonal */
        cn[7] = getpixel(pp, x - 1, y + 1, job);
        cn[5] = getpixel(pp, x + 1, y + 1, job);
        cm = getpixel(pp, x, y, job);
        ...
        cn[0] = getpixel(pp, x - 2, y, job);
        cn[4] = getpixel(pp, x + 2, y, job); /* horizontal */
        cn[2] = getpixel(pp, x, y - 2, job);
        cn[6] = getpixel(pp, x, y + 2, job); /* vertical */
        cn[1] = getpixel(pp, x - 2, y - 2, job);
        cn[3] = getpixel(pp, x + 2, y - 2, job); /* diagonal */
        cn[7] = getpixel(pp, x - 2, y + 2, job);
        cn[5] = getpixel(pp, x + 2, y + 2, job);
        /* check for 5 other and 3 same surrounding pixels */
        for (i0 = 0; i0 < 8; i0++)
          if ((cn[i0] < cs) == (cm < cs) &&
              (cn[(i0 + 7) & 7] < cs) != (cm < cs))
...
int remove_pictures(job_t *job) {
  ...
  j = 0;
  j2 = 0;
  if (job->cfg.verbose) for_each_data(&(job->res.boxlist)) {
  ...
  /* remove table frames */
  if (job->res.numC > 8) for_each_data(&(job->res.boxlist)) {
      box2 = (struct box *)list_get_current(&(job->res.boxlist));
      if (box2->c == PICTURE && box2->num_ac == 0     /* dont remove barcodes */
  ...
  if (job->res.numC > 1) /* dont remove the only char */
    for_each_data(&(job->res.boxlist)) {
      ...
      if (j > 2) { /* ToDo: check corner pixel */
        int cs = job->cfg.cs;
        j = 0;
        if (getpixel(box2->p, box2->x0, box2->y0, job) < cs) j++;
        if (getpixel(box2->p, box2->x1, box2->y0, job) < cs) j++;
        if (getpixel(box2->p, box2->x0, box2->y1, job) < cs) j++;
        if (getpixel(box2->p, box2->x1, box2->y1, job) < cs) j++;
        if (j > 2) {
          list_del(&(job->res.boxlist), box2);
          free_box(box2);
          num_del++;
  ...
  for (j = 1; j;) {
    j = 0; /* this is only because list_del does not work */
...
int remove_melted_serifs(job_t *job, pix *pp) {
  ...
  if (vvv) {
    fprintf(stderr, "# searching melted serifs ...");
  }
  for_each_data(&(job->res.boxlist)) {
    box2 = (struct box *)list_get_current(&(job->res.boxlist));
    ...
    /* upper serifs */
    for (j1 = x0; j1 + 4 < x1;) {
      j1 += loop(pp, j1, y0, x1 - x0, cs, 0, RI, job);
      x = loop(pp, j1, y0, x1 - x0, cs, 1, RI, job);
      if (j1 + x > x1 + 1) break;
      y = loop(pp, j1, y0 + 1, x1 - x0, cs, 1, RI, job);
      if (y > x) x = y;
      if (j1 + x > x1 + 1) break;
      /* measure mean thickness of serif pos: (j1,y0)-(j1+x,y0) */
      for (j2 = j3 = j4 = 0, i2 = j1; i2 < j1 + x; i2++) {
        /* 2009-07: bug, j1 used instead of i2 */
        i3 = loop(pp, i2, y0, y1 - y0, cs, 0, DO, job);
        if (8 * i3 > y1 - y0) break;
        i3 += loop(pp, i2, y0 + i3, y1 - y0, cs, 1, DO, job);
        if (8 * i3 > y1 - y0) continue;
        if (8 * i3 < y1 - y0) {
          j2 += i3;
          j3++;
        } /* sum vert. thickness */
      ...
      if (j3 == 0) {
        j1 += x;
        continue;
      } /* no serif, skip this object */
      y = y0 + (j2 + j3 - 1) / j3 +
          (y1 - y0 + 1) / 32; /* y0 + mean thickness + dy/32 + 1 */
      if (vvv & 1)
        fprintf(stderr,
                "\n#  upper serif x0,y0,j1-x0+x,y-y0 %4d %4d %3d + %2d %2d", x0,
                y0, j1 - x0, x, y - y0);

      /* check if really melted serifs */
      if (loop(pp, j1, y, x1 - x0, cs, 0, RI, job) < 1) {
        j1 += x;
        continue;
      }
      if (num_cross(j1, j1 + x, y, y, pp, cs, job) < 2) {
        j1 += x;
        continue;
      }
      if (vvv & 1) fprintf(stderr, " ok1");
      j2 = j1 + loop(pp, j1, y, x1 - x0, cs, 0, RI, job);
      j2 = j2 + loop(pp, j2, y, x1 - x0, cs, 1, RI, job);
      i3 = loop(pp, j2, y, x1 - x0, cs, 0, RI, job);
      if (i3 < 2) {
        j1 += x;
        continue;
      }
      j2 += i3 / 2;
      j3 = j2 + loop(pp, j2, y, x1 - j2, cs, 0, RI, job);
      i3 = j2 + loop(pp, j2, y + 1, x1 - j2, cs, 0, RI, job);
      if (i3 > j3) j3 = i3;
      j3 = j3 + loop(pp, j3, y, x1 - j3, cs, 1, RI, job);
      i3 = loop(pp, j3, y, x1 - j3, cs, 0, RI, job);
      if (i3 < 2 || j3 >= j1 + x) {
        j1 += x;
        continue;
      }
      ...
      if (x > 5) {
        ...
        for (y = 0; y < (y1 - y0 + 1 + 4) / 8; y++)
          put(pp, j2, y0 + y, 255, 128 + 64); /* clear highest bit */
        if (vvv & 4) {
          fprintf(stderr, "\n");
          out_x(box2, job);
        ...
        for (xb = 0, xa = 0; xa < (x1 - x0 + 4) / 8;
             xa++) { /* detect vertical gap */
          i3 = y1;
          if (box2->m3 > y0 && 2 * y1 > box2->m3 + box2->m4)
            i3 = box2->m3; /* some IJ */
          if (loop(pp, j2 - xa, i3, i3 - y0, cs, 0, UP, job) >
                  (y1 - y0 + 1) / 2 &&
              loop(pp, j2, (y0 + y1) / 2, xa + 1, cs, 0, LE, job) >= xa) {
            xb = -xa;
            break;
          }
          if (loop(pp, j2 + xa, i3, i3 - y0, cs, 0, UP, job) >
                  (y1 - y0 + 1) / 2 &&
              loop(pp, j2, (y0 + y1) / 2, xa + 1, cs, 0, RI, job) >= xa) {
            xb = xa;
            break;
          }
        }
        if (get_bw(j2, j2, y0, (y0 + y1) / 2, pp, cs, 1, job) == 0 &&
            get_bw(j2 + xb, j2 + xb, (y0 + y1) / 2, i3, pp, cs, 1, job) ==
                0) { /* divide */
          box3 = malloc_box(box2);
          box3->x1 = j2 - 1;
          box2->x0 = j2 + 1;
          x1 = box2->x1;
          cut_box(box2, job); /* cut vectors outside the box, see box.c */
          cut_box(box3, job);
          box3->num = job->res.numC;
          list_ins(&(job->res.boxlist), box2, box3);
          job->res.numC++;
          ii++; /* insert box3 before box2 */
          if (vvv & 4) fprintf(stderr, " => splitted");
          j1 = x0 = box2->x0;
          x = 0; /* hopefully ok, UVW */
        }
      }
      j1 += x;
    }
     ...
     */
    for (j1 = x0; j1 < x1;) {
      j1 += loop(pp, j1, y1, x1 - x0, cs, 0, RI, job);
      x = loop(pp, j1, y1, x1 - x0, cs, 1, RI, job);
      if (j1 + x > x1 + 1) break;
      y = loop(pp, j1, y1 - 1, x1 - x0, cs, 1, RI, job);
      if (y > x) x = y;
      if (j1 + x > x1 + 1) break;
      /* measure mean thickness of serif */
      for (j2 = j3 = j4 = 0, i2 = j1; i2 < j1 + x; i2++) {
        /* 2009-07: bug, j1 used instead of i2 */
        i3 = loop(pp, i2, y1, y1 - y0, cs, 0, UP, job);
        if (8 * i3 > y1 - y0) break;
        i3 += loop(pp, i2, y1 - i3, y1 - y0, cs, 1, UP, job);
        if (8 * i3 > y1 - y0) continue;
        if (8 * i3 < y1 - y0) {
          j2 += i3;
          j3++;
      ...
      if (vvv & 1)
        fprintf(stderr,
                "\n#  lower serif x0,y0,j1-x0+x,y1-y %4d %4d %3d + %2d %2d", x0,
                y0, j1 - x0, x, y1 - y);
      ...
      if (x > 5) {
        ...
        if (vvv & 4) {
          ...
          out_x(box2, job);
          fprintf(stderr, "# melted serifs corrected on %d %d j1=%d j3=%d",
                  j2 - x0, y - y0, j1 - x0, j3 - x0);
        }
        for (xb = 0, xa = 0; xa < (x1 - x0 + 4) / 8;
             xa++) { /* detect vertical gap */
          if (loop(pp, j2 - xa, y0, y1 - y0, cs, 0, DO, job) >
                  (y1 - y0 + 1) / 2 &&
              loop(pp, j2, (y0 + y1) / 2, xa + 1, cs, 0, LE, job) >= xa) {
            xb = -xa;
            break;
        ...
        if (get_bw(j2, j2, (y0 + y1) / 2, y1, pp, cs, 1, job) == 0 &&
            get_bw(j2 + xb, j2 + xb, y0, (y0 + y1) / 2, pp, cs, 1, job) ==
                0) { /* divide */
          box3 = malloc_box(box2);
          box3->x1 = j2 - 1;
          box2->x0 = j2;
          x1 = box2->x1;
          cut_box(box2, job); /* cut vectors outside the box */
          cut_box(box3, job);
          box3->num = job->res.numC;
          list_ins(&(job->res.boxlist), box2, box3);
          job->res.numC++;
...
int remove_rest_of_dust(job_t *job) {
  ...
  if (vvv) fprintf(stderr, "# remove_rest_of_dust (avX,nC), ... ");
  ...
  for_each_data(&(job->res.boxlist)) {
    ...
    y0 = box2->y0;
    y1 = box2->y1; /* box */
    if (box2->m2 && 4 * y0 > box2->m2 + 3 * box2->m3 &&
        2 * y1 < box2->m3 + box2->m4)
      continue;  // dont remove . 2010-10-09 qemu
    /* remove tiny box2 if to far away from bigger boxes */
    /* ToDo: remove clouds of tiny pixels (count near small, compare with num
     * bigger) */
