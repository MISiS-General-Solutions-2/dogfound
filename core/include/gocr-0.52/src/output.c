...
void out_b(struct box *px, pix *b, int x0, int y0, int dx, int dy, int cs,
  ...
  if (px) { /* overwrite rest of arguments */
    ...
    fprintf(stderr,
            ...
            " line=%d m= %d %d %d %d",
            px->dots, px->num_boxes, px->num_subboxes, decode(px->c, ASCII),
            decode(px->modifier, ASCII), px->line, px->m1 - px->y0,
            px->m2 - px->y0, px->m3 - px->y0, px->m4 - px->y0);
    if (px->num_frames) {
      int i, j, jo;
      fprintf(
      ...
      for (jo = j = i = 0; i < px->num_frames; i++, jo = j) {
        fprintf(stderr, "\n#  frame %d (%+4d,%3d,%2d) ", i, px->frame_vol[i],
                px->frame_per[i], px->num_frame_vectors[i] - jo);
        /* print only the first vectors of each frame */
        for (; j < px->num_frame_vectors[i] && j < MaxFrameVectors; j++)
          fprintf(stderr, " #%02d %2d %2d", j, px->frame_vector[j][0] - px->x0,
  ...
  if (dx > 0)
    for (y = yy0; y < yy0 + dy; y += ty) { /* reduce the output to max 78x40 */
        ...
        for (x = x0; x < x0 + dx; x += tx) { /* by merging sub-pixels */
          ...
          for (y2 = y; y2 < y + ty && y2 < y0 + dy; y2++) /* sub-pixels */
            for (x2 = x; x2 < x + tx && x2 < x0 + dx; x2++) {
              if ((getpixel(px->p, x2 - x0 + px->x0, y2 - y0 + px->y0, job) <
                   cs))
                c1 = '@';
            }
      ...
      /* mark lines with < */
      if (px)
        if (y - y0 + px->y0 == px->m1 || y - y0 + px->y0 == px->m2 ||
            y - y0 + px->y0 == px->m3 || y - y0 + px->y0 == px->m4)
          c1 = '<';
...
void out_x(struct box *px, job_t *job) {
  out_b(px, NULL, 0, 0, 0, 0, job->cfg.cs, job);
}
...
void out_x2(struct box *box1, struct box *box2, job_t *job) {
  ...
  ty = (box1->y1 - box1->y0) / 40 +
       1; /* step, usually 1, but greater on large maps */
  ...
  for (i = 0; i < dy; i += ty) { /* reduce the output to max 78x40??? */
    ...
    for (x = box1->x0; x <= box1->x1; x += tx)
      fprintf(stderr, "%c",
              c1[((getpixel(b, x, y, job) < job->cfg.cs) ? 0 : 8) +
                 marked(b, x, y)]);
    if (!box2) continue;
    fprintf(stderr, "  ");
    y = box2->y0 + i;
  ...
  for_each_data(&(job->res.boxlist)) {
    ...
    for (j = 0; j < box2->num_ac; j++)
      if (!lc || (box2->tac[j] && strchr(lc, box2->tac[j])) ||
          (box2->tas[j] && strstr(lc, box2->tas[j])))
        break;
    if (j < box2->num_ac)
    ...
    if (!lc || (strchr(lc, box2->c) && box2->c < 256 && box2->c) ||
      ...
      if (!pp) pp = box2->p;
      fprintf(stderr,
              "\n# list shape %3d x=%4d %4d d= %3d %3d vf=%d ac=%d %04x %s", i,
              box2->x0, box2->y0, box2->x1 - box2->x0 + 1,
              box2->y1 - box2->y0 + 1, box2->num_frames, box2->num_ac,
              (int)box2->c, /* wchar_t -> char ???? */
              decode(box2->c, ASCII));
      if (job->cfg.verbose & 4) out_x(box2, job);
    }
    i++;
  }
...
int debug_img(char *fname, struct job_s *job, int opt) {
    ...
    for_each_data(&(job->res.boxlist)) { /* char boxes */
      ...
      if (box2->x0 > 1)
        for (y = box2->y0; y <= box2->y1; y++) {
          np = &pp->p[box2->x0 - 1 + y * pp->x];
          if (*np < 160) continue;
          *np |= col;
        }
      if (box2->y1 + 1 < pp->y)
        for (x = box2->x0; x <= box2->x1; x++) {
          np = &pp->p[x + (box2->y1 + 1) * pp->x];
          if (*np < 160) continue;
          *np |= col;
        }
  ...
  if (opt & 4) {
    ...
    int yr;
    if (job->cfg.verbose) fprintf(stderr, "# mark lines for %s.ppm\n", fname);
    ...
    for (i = 0; i < lines->num; i++) { /* mark lines by 0x08 = blue */
      ...
      for (j = -1; j < dx + 1; j++) {
        ...
        for (y = lines->m1[i]; y <= lines->m4[i]; y++) {
          /* box arround m2-m3 */
          if (y >= lines->m2[i] && y <= lines->m3[i] && j > -1 && j < dx)
            continue;
          yr = y; /* y.rotated */
