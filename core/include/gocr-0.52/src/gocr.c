...
static void out_version(int v) {
  fprintf(stderr,
          " Optical Character Recognition --- gocr " version_string
          " " release_string
          "\n"
  ...
  if (v) fprintf(stderr, " use option -h for help\n");
  if (v & 2) exit(1);
  return;
}
...
static void help(void) {
  ...
  fprintf(stderr, /* string length less than 509 bytes for ISO C89 */
          ...
          " -C string - char filter (ex. hexdigits: "
          "0-9A-Fx"
          ", only ASCII)\n"
    ...
    if (argv[i][0] == '-' && argv[i][1] != 0) {
      ...
      switch (argv[i][1]) {
...
#else
            fprintf(stderr,
                    "stderr redirection not possible without unistd.h\n");
#endif
        ...
        case 'f': /* output format */
          if (strcmp(s1, "ISO8859_1") == 0)
            job->cfg.out_format = ISO8859_1;
          else if (strcmp(s1, "TeX") == 0)
            job->cfg.out_format = TeX;
          else if (strcmp(s1, "HTML") == 0)
            job->cfg.out_format = HTML;
          else if (strcmp(s1, "XML") == 0)
            job->cfg.out_format = XML;
          else if (strcmp(s1, "SGML") == 0)
            job->cfg.out_format = SGML;
          else if (strcmp(s1, "UTF8") == 0)
            job->cfg.out_format = UTF8;
          else if (strcmp(s1, "ASCII") == 0)
...
static void mark_start(job_t *job) {
  ...
  if (job->cfg.verbose) {
    ...
    fprintf(stderr, "\n");
    fprintf(stderr,
            "# options are: -l %d -s %d -v %d -c %s -m %d -d %d -n %d -a %d -C "
            "\"%s\"\n",
            job->cfg.cs, job->cfg.spc, job->cfg.verbose, job->cfg.lc,
            job->cfg.mode, job->cfg.dust_size, job->cfg.only_numbers,
            job->cfg.certainty, job->cfg.cfilter);
...
void print_output(job_t *job) {
  ...
  /* TODO: replace getTextLine-loop(line) by output_text() (libs have to use
     pipes) simplify code 2010-09-26
  */
  linecounter = 0;
...
char *get_concatenated_textlines_and_free_textlines(job_t *job) {
  const char *line;
  int linecounter = 0;
  ...
  */
  ...
  while (line) {
    ...
    if (result == NULL) {
      ...
      temp = malloc(strlen(result) + strlen(line) + 1);
      strcpy(temp, result);
      strcat(temp, line);
    }
    free(result);
...
void populateJobWithPgm(job_t *job, int datalen, char *data, int argn,
  ...
  readPgmFromMemory(datalen, data, &job->src.p, 0);
}
