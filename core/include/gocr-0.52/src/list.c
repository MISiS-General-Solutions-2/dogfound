...
int list_ins(List *l, void *data_after, void *data) {
  ...
  if (!data_after || !l->n) return list_app(l, data);

  /* get data_after element */
  if (!(after_element = list_element_from_data(l, data_after))) return 1;

  /* alloc a new element */
  if (!(e = (Element *)malloc(sizeof(Element)))) return 1;
  e->data = data;
...
Element *list_element_from_data(List *l, void *data) {
  ...
  if (!l || !data || !l->n) return NULL;

  temp = l->start.next;

  while (temp->data != data) {
    if (!temp || temp == &l->stop) return NULL;
    temp = temp->next;
  }
  return temp;
}
...
int list_del(List *l, void *data) {
  ...
  if (!(temp = list_element_from_data(l, data))) return 1;
...
void list_lower_level(List *l) {
  ...
  l->level--;
  g_debug(
      fprintf(stderr, "LEV5: level--=%d current[]=%p\n", l->level, l->current);)
}
...
void *list_next(List *l, void *data) {
  ...
  if (!l || !(temp = list_element_from_data(l, data))) return NULL;
...
void list_sort(List *l, int (*compare)(const void *, const void *)) {
  ...
  for (i = 0; i < l->n; i++) {
    ...
    for (temp = l->start.next->next; temp != NULL && temp != &l->stop;
         temp = temp->next) {
...
int list_and_data_free(List *l, void (*free_data)(void *data)) {
  ...
  for_each_data(l) {
    if ((data = list_get_current(l))) free_data(data);
  }
  end_for_each(l);
