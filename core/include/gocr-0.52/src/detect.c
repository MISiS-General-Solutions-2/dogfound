 ...
 */
int detect_lines1(pix *p, int x0, int y0, int dx, int dy, job_t *job) {
  int i, jj, /*j2,*/ y, yy, my, mi, /* mc,*/ i1, i2, i3, i4, m1, m2, m3, m4,
      ma1, ma2, ma3, ma4, m3pre, m4pre, mt1, mt2, mt4,
      mtc;                        /* 2017 from global triple-pxH */
  ...
  i = lines->num;
  if (dy < 4) return 0; /* image is to low for latin chars */
  my = jj = 0;
  // get the mean height of all hollow chars
  // (better than mean value of everything including bg-pattern or dust?)
  for_each_data(&(job->res.boxlist)) {
    box2 = (struct box *)list_get_current(&(job->res.boxlist));
    if (box2->c != PICTURE && box2->num_frames > 1 &&
        box2->num_frames < 4 /* 1 or 2 holes */
        && box2->y0 >= y0 && box2->y1 <= y0 + dy && box2->x0 >= x0 &&
        box2->x1 <= x0 + dx && box2->frame_vol[0] > 0 &&
        box2->frame_vol[1] < 0) {
      jj++;
  ...
  if (jj == 0) {
    ...
    for_each_data(&(job->res.boxlist)) {
      box2 = (struct box *)list_get_current(&(job->res.boxlist));
      if (box2->c != PICTURE && box2->y1 - box2->y0 + 1 >= 4 /* 4x6 font */
          && box2->y0 >= y0 && box2->y1 <= y0 + dy && box2->x0 >= x0 &&
          box2->x1 <= x0 + dx) {
        jj++;
        my += box2->y1 - box2->y0 + 1;
  ...
  if (jj == 0) return 0; /* no chars detected */
  ...
  if (my < 4) return 0; /* mean high is to small => error */

    //  ToDo: find 2 neighbouring chars per line pH Hp + third x or p or H to
    //  verify
  ...
  for_each_data(&(job->res.boxlist)) { /* 2017-07 added , ToDo: use ma1-ma4 */
    ...
    for_each_data(&(job->res.boxlist)) {
      ...
      if (box2->y0 != box3->y0 || box2->y1 <= box3->y1) continue; /* px-like */
      for_each_data(&(job->res.boxlist)) {
        ...
        if (box3->y1 != box4->y1 || box3->y0 <= box4->y0)
          continue; /* pxH-like */
                    //        if (job->cfg.verbose & 16)
        //          fprintf(stderr,"  found pH-like-pair xy %3d %3d  %3d %3d %3d
        //          %3d\n#",
  ...
  if (job->cfg.verbose & 16)
    fprintf(stderr, "  found %4d pxH-like-triples m124=%4d %4d %4d\n#", mtc,
            (mt1 - mtc + 1) / mtc, (mt2 - mtc + 1) / mtc,
            (mt4 + mtc - 1) / mtc);
  mt1 = (mt1 - mtc + 1) / mtc; /* round up! */
  mt2 = (mt2 - mtc + 1) / mtc;
  mt4 =
      (mt4 + mtc - 1) / mtc; /* added 2017-07 ToDo: use for short lines later */
  ...
  for (/*j2 =*/y = y0; y < y0 + dy; y++) { /* line by line loop */
    ...
    for_each_data(&(job->res.boxlist)) {
      box2 = (struct box *)list_get_current(&(job->res.boxlist));
      if (box2->line > 0 || box2->c == PICTURE) continue;
      if (box2->y0 < y0 || box2->y1 >= y0 + dy || box2->x0 < x0 ||
          box2->x1 >= x0 + dx)
        continue;  // out of image
      if (lines->dx)
        yy = lines->dy * box2->x0 / (lines->dx);
      else
        yy = 0;
      ...
      if (box2->y0 >= y + yy  // lower than y
        ...
        if (lines->x1[i] < box2->x1) lines->x1[i] = box2->x1; /* right end */
        if (lines->x0[i] > box2->x0) lines->x0[i] = box2->x0; /* left end */
      }
      /* check for ij-dots, used if chars of same high */
      if (box2->y1 - box2->y0 + 1 <= 2 * my  // ~ barcode or small images
          && box2->y0 >= y + yy
          // && box2->y0 >= y
          && box2->y1 < m1 + yy + my / 4 && box2->y0 < mi + yy) {
        mi = box2->y0 - yy; /* highest upper boundary i-dot */
      }
      // fprintf(stderr,"\n check %3d %3d-%3d y=%d yy=%d m1=%d", box2->x0,
      // box2->y0, box2->y1, y, yy, m1);
      ...
      if (box2->y0 >= y + yy                         // lower than y
        ...
        if (box2->y0 > m2 + yy) {
          ...
          if (job->cfg.verbose & 16)
            fprintf(stderr,
                    "  set m2= %3d %+3d yy= %2d  reason1 xy= %4d %4d\n#", m2,
                    m2 - m1, yy, box2->x0, box2->y0);
        }
        if (box2->y1 > m4 + yy && (my > 6 || box2->y1 < m3 + my)) {
          m4 = box2->y1 -
               yy; /* lowest lower boundary, small font lines can touch */
          if (job->cfg.verbose & 16)
            fprintf(stderr,
                    "  set m4= %3d %+3d yy= %2d  reason1 xy= %4d %4d\n#", m4,
                    m4 - m1, yy, box2->x0, box2->y0);
        }
        if (box2->y1 < m3 + yy &&
            ((2 * box2->y1 > m2 + m4 + yy && m2 > m1) ||
             (4 * box2->y1 >
              m1 + 3 * m4 + yy)))  // care for TeX: \(^1\)Footnote 2003
          /* "'!?" could cause trouble here, therefore this lines */
          /* ToDo: get_bw costs time, check pre and next */
          if (get_bw(box2->x0, box2->x1, box2->y1 + 1, box2->y1 + my / 2,
                     box2->p, job->cfg.cs, 1, job) == 0 ||
              get_bw(box2->x0, box2->x1, box2->y1 + my / 2, box2->y1 + my / 2,
                     box2->p, job->cfg.cs, 1, job) == 1 ||
              num_cross(box2->x0, box2->x1, (box2->y0 + box2->y1) / 2,
                        (box2->y0 + box2->y1) / 2, box2->p, job->cfg.cs,
                        job) > 2) {
            m3 = box2->y1 - yy; /* highest lower boundary */
            if (job->cfg.verbose & 16)
              fprintf(stderr,
                      "  set m3= %3d %+3d yy= %2d  reason1 xy= %4d %4d\n#", m3,
                      m3 - m1, yy, box2->x0, box2->y0);
    ...
    if (job->cfg.verbose & 16)
      fprintf(stderr,
              "  L%02d step 1 y=%4d m= %4d %+3d %+3d %+3d x= %3d %+3d"
              " my=%2d chars=%3d\n#",
    ...
    for_each_data(&(job->res.boxlist)) {
      ...
      if (box2->y0 >= y + yy && box2->y1 < y0 + dy  // lower than y
        ...
        if (box2->y0 - yy >= m1 - my / 4 && box2->y0 - yy <= m2 + my / 4 &&
            box2->y1 - yy >= m3 - my / 4 &&
    ...
    for_each_data(&(job->res.boxlist)) {
      ...
      if (lines->dx)
        ...
        yy = 0;
      if (1
          //        && box2->y1 >= m1-1 && box2->y0 <= m3+2  // 0811qemu2
          //        2010-10-01
          ...
          && box2->y0 <= m4 + 2 && box2->y1 >= m1 - 1  // -1 for `` in 0811qemu2
    ...
    if (job->cfg.verbose & 16)
      fprintf(stderr,
              "  L%02d step 2 y=%4d m= %4d %+3d %+3d %+3d x= %3d %+3d\n#", i, y,
              m1, m2 - m1, m3 - m1, m4 - m1, lines->x0[i],
              lines->x1[i] - lines->x0[i]);
#endif

    if (m4 == m1) {
      if (m3 + m4 > 2 * y)
        y = (m4 + m3) / 2; /* lower end may overlap the next line */
      continue;
    }
    jj = 0;
    lines->wt[i] = 100;
    ...
    if (jj > 0 && job->cfg.verbose) {
      ...
      fprintf(stderr,
              "  L%02d w= %3d y= %3d m= %4d %+3d %+3d %+3d (H,x,base,p)\n#", i,
              lines->wt[i], y, m1, m2 - m1, m3 - m1, m4 - m1);
      fprintf(stderr, "  L%02d        y= %3d i=  %3d %3d %3d %3d (counts)\n#",
              i, y, i1, i2, i3, i4);
      if (jj == 3)
        fprintf(stderr, "  all boxes of same high! mi=%d\n#", mi - m1);
    ...
    if (mt1 < mt2 && mt2 < 0 && mt4 > 0 &&
        jj == 3) { /* valid Hxp-tripples found, use here */
      ...
      if (jj == 0)
        fprintf(stderr, "       fixed by Hxp to %4d %3d %3d %3d\n#", m1,
                m2 - m1, m3 - m1, m4 - m1);
    }
    ...
    if (jj == 3) m2 = m1 + (m3 - m1) / 4 + 1; /* expect: 0123456789ABCDEF */
    if ((m2 - m1) < 2) m2 = m1 + 2;           /* font hight < 8 pixel ? */
    if (jj & 2) m4 = m3 + (m4 - m1) / 4 + 1;  /* chars have same lower base */
    ...
    {                                 // empty space between lines
      ...
      if (job->cfg.verbose & 16)
        fprintf(
            stderr,
            "  L%02d final  y= %3d m= %4d %+3d %+3d %+3d x= %3d %+3d w= %d\n#",
            i, y, m1, m2 - m1, m3 - m1, m4 - m1, lines->x0[i],
            lines->x1[i] - lines->x0[i], lines->wt[i]);
      if (i < MAXlines && m4 - m1 > 4) i++;
      if (i >= MAXlines) {
    ...
    if (m3 + m4 > 2 * y)
      y = (m3 + m4) / 2; /* lower end may overlap the next line */
    if (m3 > m3pre)
      m3pre = m3;
    else
      m3 = y0; /* set for next-line scan */
    if (m4 > m4pre)
      m4pre = m4;
    else
      m4 = y0; /* set for next-line scan */
  }
  lines->num = i;
  if (job->cfg.verbose)
...
int detect_lines2(pix *p, int x0, int y0, int dx, int dy, int r, job_t *job) {
  ...
  for_each_data(&(job->res.boxlist)) {
    box3 = (struct box *)list_get_current(&(job->res.boxlist));
    if (box3->y0 >= y0 && box3->y1 < y0 + dy && box3->x0 >= x0 &&
        box3->x1 < x0 + dx) {
  ...
  if (r < 8) {  // max. depth
            ...
            if (box3 != box2)
              if (box3->y0 >= y0 && box3->y1 < y0 + dy)
                if (box3->x0 >= x0 && box3->x1 < x0 + dx)
                  if (box3->c !=
                      PICTURE) /* ToDo: not sure, that this is a good idea */
    ...
    if (x3 > 0 || y3 > 0) {
      if (x3 > mx && x3 > 2 * y3 && (dy > 5 * x3 || (x3 > 10 * y3 && y3 > 0)))
        i = 1;
      else if (dx > 5 * y3 && y3 > my)
        i = 2;
    }
    ...
    for_each_data(&(job->res.boxlist)) {  // not very efficient, sorry
      ...
      if (box2->c == PICTURE)
        if (box2->y0 >= y0 && box2->y1 < y0 + dy && box2->x0 >= x0 &&
            box2->x1 < x0 + dx) {  // hline ???
          // largest gap:                  width position
          if (box2->x1 - box2->x0 + 4 > dx && box2->y1 + 4 < y0 + dy) {
            y3 = 1;
            y2 = box2->y1 + 1;
            i = 2;
            break;
          }
          if (box2->x1 - box2->x0 + 4 > dx && box2->y0 - 4 > y0) {
            y3 = 1;
            y2 = box2->y0 - 1;
            i = 2;
            break;
          }
          if (box2->y1 - box2->y0 + 4 > dy && box2->x1 + 4 < x0 + dx) {
            x3 = 1;
            x2 = box2->x1 + 1;
            i = 1;
            break;
          }
          if (box2->y1 - box2->y0 + 4 > dy && box2->x0 - 4 > x0) {
            x3 = 1;
            x2 = box2->x0 - 1;
            i = 1;
            break;
    ...
    if (job->cfg.verbose && i)
      fprintf(stderr, " divide at %s x=%4d y=%4d dx=%4d dy=%4d",
              ((i) ? ((i == 1) ? "x" : "y") : "?"), x2, y2, x3, y3);
    // divide horizontally if v-gap is thicker than h-gap
    // and length is larger 5*width
    if (i == 1) {
      detect_lines2(p, x0, y0, x2 - x0 + 1, dy, r + 1, job);
      return detect_lines2(p, x2, y0, x0 + dx - x2 + 1, dy, r + 1, job);
    }
    // divide vertically
    if (i == 2) {
      detect_lines2(p, x0, y0, dx, y2 - y0 + 1, r + 1, job);
      return detect_lines2(p, x0, y2, dx, y0 + dy - y2 + 1, r + 1, job);
    }
  }

  if (job->cfg.verbose)
    if (dx < 5 || dy < 7) fprintf(stderr, " empty box");
  if (dx < 5 || dy < 7) return 0;  // do not care about dust
  if (job->cfg.verbose)
    fprintf(stderr, " box detected at %4d %4d %4d %4d", x0, y0, dx, dy);
  if (job->tmp.ppo.p) {
  ...
  return detect_lines1(p, x0 - 0 * 1, y0 - 0 * 2, dx + 0 * 2, dy + 0 * 3, job);
...
int detect_rotation_angle(job_t *job) {
  struct box *box2, *box3, *box_nn; /* nearest neighbour box */
  int x2, y2, x3, y3, dist, mindist, pass, rx = 0, ry = 0,
                                           re = 0,  // final result
      /* to avoid 2nd run, wie store pairs in 2 different categories */
      nn[4] = {0, 0, 0,
               0}, /* num_pairs used for estimation [(pass-1)%2,pass%2] */
      dx[4] = {0, 0, 0, 0}, /* x-component of rotation vector per pass */
      dy[4] = {0, 0, 0, 0}, /* y-component of rotation vector per pass */
      er[4] = {INorm / 4, 0, 0,
               0}; /* mean angle deviation to pass-1 (radius^2) */
  ...
  for (pass = 0; pass < 4; pass++) {
    for_each_data(&(job->res.boxlist)) {
      ...
      for_each_data(&(job->res.boxlist)) {
        ...
        if (pass > 0) {  // new variant = scalar product
          ...
          re =
              (int)((1. * (x3 - x2) * dx[pass - 1] + (y3 - y2) * dy[pass - 1]) *
                    (1. * (x3 - x2) * dx[pass - 1] + (y3 - y2) * dy[pass - 1]) *
                    INorm /
                    (1. * ((x3 - x2) * (x3 - x2) + (y3 - y2) * (y3 - y2)) *
                     (1. * dx[pass - 1] * dx[pass - 1] +
                      dy[pass - 1] * dy[pass - 1])));
          if (INorm - re > er[pass - 1]) continue;  // hits mean deviation
        }
        /* neighbours should have same order of size (?) */
        if (3 * (box3->y1 - box3->y0 + 4) < 2 * (box2->y1 - box2->y0 + 1))
          continue;
        if (2 * (box3->y1 - box3->y0 + 1) > 3 * (box2->y1 - box2->y0 + 4))
          continue;
        if (2 * (box3->x1 - box3->x0 + 1) > 5 * (box2->x1 - box2->x0 + 4))
          continue;
        if (5 * (box3->x1 - box3->x0 + 4) < 2 * (box2->x1 - box2->x0 + 1))
          continue;
        /* should be in right range, Idea: center3 outside box2? noholes */
        if ((x3 < box2->x1 - 1) && (x3 > box2->x0 + 1) && (y3 < box2->y1 - 1) &&
            (y3 > box2->y0 + 1))
          continue;
        // if chars are of different size, connect careful
        if (abs(x3 - x2) > 2 * (box2->x1 - box2->x0 + box3->x1 - box3->x0 + 2))
          continue;
        if (abs(y3 - y2) > (box2->x1 - box2->x0 + box3->x1 - box3->x0 + 2))
          continue;
        dist = (y3 - y2) * (y3 - y2) + (x3 - x2) * (x3 - x2);
        // make distances in pass-1 directions shorter or continue if not in
        // pass-1 range?
      ...
      if (pass > 0 &&
          16 * abs(dy[pass - 1]) < dx[pass - 1])  // dont work for strong rot.
        if (abs(box2->y1 - box2->y0 - box3->y1 + box3->y0) >
            (box2->y1 - box2->y0) / 8) {
          // ad eh ck ...
          if (abs(box2->y1 - box3->y1) < abs(y3 - y2)) {
            y2 = box2->y1;
            y3 = box3->y1;
          }
          // ag ep qu ...
          if (abs(box2->y0 - box3->y0) < abs(y3 - y2)) {
            y2 = box2->y0;
            y3 = box3->y0;
          }
        }
    ...
    if (job->cfg.verbose)
      fprintf(stderr,
              "# rotation angle (x,y,maxr,num)"
              " %6d %6d %6d %4d pass %d\n",
              rx, ry, er[pass], nn[pass], pass + 1);
  }
  if (abs(ry * 100) > abs(rx * 50))
    fprintf(stderr,
            "<!-- gocr will fail, strong rotation angle detected -->\n");
...
/* ----- detect lines --------------- */
int detect_text_lines(pix *pp, int mo, job_t *job) {
  int vvv = job->cfg.verbose; /* fixme (global var) */

  if (vvv)
    fprintf(stderr, "# detect.c detect_text_lines (vvv=16 for more info)\n");
  if (mo & 4) {
    if (vvv) fprintf(stderr, "# zoning\n# ... ");
    detect_lines2(pp, 0, 0, pp->x, pp->y, 0,
                  job);  // later replaced by better algo
    if (vvv) fprintf(stderr, "\n");
  } else
    detect_lines1(pp, 0, 0, pp->x, pp->y, job);  // old algo
  return 0;
}
...
int adjust_text_lines(pix *pp, int mo, job_t *job) {
  ...
  if (dx) for_each_data(&(job->res.boxlist)) {
      box2 = (struct box *)list_get_current(&(job->res.boxlist));
      ...
      if (strchr("aemnr", (char)box2->tac[0])) {  // cC vV sS oO ... is unsure!
        m[box2->line * 16 + 1] += y0;
        m[box2->line * 16 + 5]++;  // num m2
        m[box2->line * 16 + 2] += y1;
        m[box2->line * 16 + 6]++;  // num m3
        if (m[box2->line * 16 + 9] > y0)
          m[box2->line * 16 + 9] = y0; /* min m2 */
        if (m[box2->line * 16 + 13] < y0)
          m[box2->line * 16 + 13] = y0; /* max m2 */
        if (m[box2->line * 16 + 10] > y1)
          m[box2->line * 16 + 10] = y1; /* min m3 */
        if (m[box2->line * 16 + 14] < y1)
          m[box2->line * 16 + 14] = y1; /* max m3 */
      }
      if (strchr("bdhklABDEFGHIKLMNRT12346789",
                 (char)box2->tac[0])) {  // ~oO s5
        m[box2->line * 16 + 0] += y0;
        m[box2->line * 16 + 4]++;  // num m1
        m[box2->line * 16 + 2] += y1;
        m[box2->line * 16 + 6]++;  // num m3
        if (m[box2->line * 16 + 8] > y0)
          m[box2->line * 16 + 8] = y0; /* min m1 */
        if (m[box2->line * 16 + 12] < y0)
          m[box2->line * 16 + 12] = y0; /* max m1 */
        if (m[box2->line * 16 + 10] > y1)
          m[box2->line * 16 + 10] = y1; /* min m3 */
        if (m[box2->line * 16 + 14] < y1)
          m[box2->line * 16 + 14] = y1; /* max m3 */
      ...
      if (strchr("gq", (char)box2->tac[0])) {  // pP is unsure
        ...
        m[box2->line * 16 + 5]++;  // num m2
        m[box2->line * 16 + 3] += y1;
        m[box2->line * 16 + 7]++;  // num m4
        if (m[box2->line * 16 + 9] > y0)
          m[box2->line * 16 + 9] = y0; /* min m2 */
        if (m[box2->line * 16 + 13] < y0)
          m[box2->line * 16 + 13] = y0; /* max m2 */
        if (m[box2->line * 16 + 11] > y1)
          m[box2->line * 16 + 11] = y1; /* min m4 */
        if (m[box2->line * 16 + 15] < y1)
          m[box2->line * 16 + 15] = y1; /* max m4 */
      }
    }
  end_for_each(&(job->res.boxlist));
  ...
  for (i = 1; i < l; i++) {
    ...
    if (m[i * 16 + 4])
      diff += abs(job->res.lines.m1[i] - m[i * 16 + 0] / m[i * 16 + 4]);
    if (m[i * 16 + 5])
      diff += abs(job->res.lines.m2[i] - m[i * 16 + 1] / m[i * 16 + 5]);
    if (m[i * 16 + 6])
      diff += abs(job->res.lines.m3[i] - m[i * 16 + 2] / m[i * 16 + 6]);
    if (m[i * 16 + 7])
      diff += abs(job->res.lines.m4[i] - m[i * 16 + 3] / m[i * 16 + 7]);
    ...
    if (m[i * 16 + 4])
      job->res.lines.m1[i] =
          (m[i * 16 + 0] + m[i * 16 + 4] / 2) / m[i * 16 + 4];
    if (m[i * 16 + 5])
      job->res.lines.m2[i] =
          (m[i * 16 + 1] + m[i * 16 + 5] / 2) / m[i * 16 + 5];
    if (m[i * 16 + 6])
      job->res.lines.m3[i] =
          (m[i * 16 + 2] + m[i * 16 + 6] / 2) / m[i * 16 + 6];
    if (m[i * 16 + 7])
      job->res.lines.m4[i] =
          (m[i * 16 + 3] + m[i * 16 + 7] / 2) / m[i * 16 + 7];
    // care about very small fonts
    if (job->res.lines.m2[i] - job->res.lines.m1[i] <= 1 &&
        m[i * 16 + 5] == 0 && m[i * 16 + 4])
      job->res.lines.m2[i] = job->res.lines.m1[i] + 2;
    if (job->res.lines.m2[i] - job->res.lines.m1[i] <= 1 &&
        m[i * 16 + 4] == 0 && m[i * 16 + 5])
      job->res.lines.m1[i] = job->res.lines.m2[i] - 2;
    if (job->res.lines.m4[i] - job->res.lines.m3[i] <= 1 &&
        m[i * 16 + 7] == 0 && m[i * 16 + 6])
    ...
    if (m[i * 16 + 7] < 1 &&
        job->res.lines.m4[i] <=
            job->res.lines.m3[i] +
                (job->res.lines.m3[i] - job->res.lines.m2[i]) / 4)
      job->res.lines.m4[i] = job->res.lines.m3[i] +
                             (job->res.lines.m3[i] - job->res.lines.m2[i]) / 4;
    if (m[i * 16 + 7] < 1 && m[i * 16 + 12 + 2] > 0 &&  // m4 < max.m3+..
        job->res.lines.m4[i] <
            2 * m[i * 16 + 12 + 2] - job->res.lines.m3[i] + 2)
    ...
    if (job->cfg.verbose & 17)
      fprintf(stderr,
              ...
              job->res.lines.m4[i] - job->res.lines.m1[i], m[i * 16 + 4],
              m[i * 16 + 5], m[i * 16 + 6], m[i * 16 + 7], job->res.lines.wt[i],
              diff);
  }
  ...
  if (dx) for_each_data(&(job->res.boxlist)) {
      ...
      if (2 * box2->y1 > 2 * job->res.lines.m4[box2->line] +
        ...
        box2->line = 0;
      /* do adjustments */
      if (box2->num_ac > 0 && box2->num_ac > 31 &&
          box2->tac[0] < 127 /* islower(>256) may SIGSEGV */
          && strchr("cCoOpPsSuUvVwWxXyYzZ", (char)box2->tac[0])) {  // no_wchar
        if (box2->y0 - ((box2->x1) * dy / dx) <
                (job->res.lines.m1[box2->line] +
                 job->res.lines.m2[box2->line]) /
                    2 &&
            islower(box2->tac[0])) {
          setac(box2, toupper((char)box2->tac[0]), (box2->wac[0] + 101) / 2,
                job);
          diff++;
        }
        if (box2->y0 - ((box2->x1) * dy / dx) >
                (job->res.lines.m1[box2->line] + job->res.lines.m2[box2->line] +
                 1) /
                    2 &&
            isupper(box2->tac[0])) {
          setac(box2, tolower((char)box2->tac[0]), (box2->wac[0] + 101) / 2,
                job);
          diff++;
        }
      }
    ...
    if (box4->c != PICTURE) {
      ...
      if (job->res.avX * job->res.avY > 0) {
        if (x1 - x0 + 1 > 4 * job->res.avX && y1 - y0 + 1 > 4 * job->res.avY)
          continue; /* small picture */
        if (4 * (y1 - y0 + 1) < job->res.avY || y1 - y0 < 2)
          continue;  // dots .,-_ etc.
      }
      if (x1 - x0 + 1 < 4 && y1 - y0 + 1 < 6) continue; /* dots etc */
...
int detect_pictures(job_t *job) {
  ...
  if (job->res.numC == 0) {
    if (job->cfg.verbose)
      fprintf(stderr, "# detect.c L%d Warning: numC=0\n", __LINE__);
    return -1;
  }
