...
int copybox(pix *p, int x0, int y0, int dx, int dy, pix *b, int len,
  ...
  for (y = 0; y < dy; y++)
    for (x = 0; x < dx; x++)
      pixel_atp(b, x, y) = getpixel(p, x + x0, y + y0, job);
#endif
...
int reduce_vectors(struct box *box1, int mode) {
  ...
  if (box1->num_frames > 0)
    for (i2 = besti2; i2 < box1->num_frame_vectors[box1->num_frames - 1] - 1;
         i2++) {
      box1->frame_vector[i2][0] = box1->frame_vector[i2 + 1][0];
      box1->frame_vector[i2][1] = box1->frame_vector[i2 + 1][1];
    }
  for (i1 = besti1; i1 < box1->num_frames; i1++) box1->num_frame_vectors[i1]--;
 ...
 */
int merge_boxes(struct box *box1, struct box *box2, job_t *job) {
  int i1, i2, i3, i4;
  ...
  if ((box2->x1 - box2->x0 + 1) * (box2->y1 - box2->y0 + 1) >
    ...
    bbigger = box1;
    bsmaller = box2;
  }  // if called by glue_holes_inside_chars()
  // fprintf(stderr,"\n y bigger"); out_x(bbigger); out_x(bsmaller);
  /* ToDo: does not work if a third box is added */
  if (box2->y0 > box1->y1 || box2->y1 < box1->y0 || box2->x0 > box1->x1 ||
      box2->x1 < box1->x0) {
    ...
    if (4 * box1->y1 < 3 * box2->y0 + box2->y1 && box1->y0 < box2->y0)
      (box2->dots)++;  // 2010-09-24 :
  } else {
  ...
  while (i1 + i2 > MaxFrameVectors) {
    if (i1 > i2) {
      reduce_vectors(bbigger, 1);
      i1--;
    } else {
      reduce_vectors(bsmaller, 1);
      i2--;
    }
  }
  ...
  for (i4 = i3 = 0; i3 < bsmaller->num_frames; i3++) {
    ...
    for (; i4 < bsmaller->num_frame_vectors[i3]; i4++) {
      memcpy(tmpbox.frame_vector[i1], bsmaller->frame_vector[i4],
             2 * sizeof(int));
      i1++;
    }
  ...
  memcpy(box1->num_frame_vectors, tmpbox.num_frame_vectors,
         sizeof(int) * MaxNumFrames);
  memcpy(box1->frame_vol, tmpbox.frame_vol, sizeof(int) * MaxNumFrames);
  memcpy(box1->frame_per, tmpbox.frame_per, sizeof(int) * MaxNumFrames);
  memcpy(box1->frame_vector, tmpbox.frame_vector,
         sizeof(int) * 2 * MaxFrameVectors);
#if 0
...
int cut_box(struct box *box1, job_t *job) {
  ...
  for (i1 = 0; i1 < box1->num_frames; i1++) {
    ...
    if (box1->frame_per[i1] == 2) { /* all vectors outside */
      ...
      for (i4 = i2;
           i4 < box1->num_frame_vectors[box1->num_frames - 1] - (i3 - i2);
           i4++) {
    ...
    for (i2 = 0; i2 < box1->num_frame_vectors[i1]; i2++) {
      ...
      if (x < box1->x0 || x > box1->x1 || y < box1->y0 || y > box1->y1) {
        ...
        if (i3 == 0) { /* inside_to_outside, wrong if it starts outside */
          ...
          /* remove vector */
          if (dbg > 1)
            fprintf(stderr, "\n remove vector[%d][%d] x= %2d %2d", i1, i2,
                    x - box1->x0, y - box1->y0);
          for (i4 = i2; i4 < box1->num_frame_vectors[box1->num_frames - 1] - 1;
               i4++) {
